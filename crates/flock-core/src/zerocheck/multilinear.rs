...
mod tests {
    ...
    fn gfni_masked_prefold_matches_unpredicated_kernel() {
        ...
        // Unmasked c4 leaves (no `dead` ABI) match the masked `dead = 0` path
        // that the ranked refill actually calls.
        let mut c4u = [F128::ZERO; 64];
        let mut c4bu = [F128::ZERO; 64];
        unsafe {
            kernels::x86_64::gfni_fold64_rows_c4(clean.as_ptr(), &cm, c4u.as_mut_ptr());
            kernels::x86_64::gfni_fold64_rows_c4_bcast(clean.as_ptr(), &cm, c4bu.as_mut_ptr());
        }
        assert_eq!(c4u, c4, "unmasked c4 vs masked dead=0");
        assert_eq!(c4bu, c4b, "unmasked c4_bcast vs masked dead=0");
