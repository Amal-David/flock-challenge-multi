...
pub(crate) fn uni_skip_fold_and_round_pair_compact_padded_lookahead(
    ...
    // `write_compact = false` is the raw-K route's store elision: the
    // returned compact state keeps its allocation and recorded length but
    // its contents are never written (and never read — the caller feeds the
    // K pass from the original rows; see `fold2_raw_and_round45_into`).
    // Messages and the lookahead are identical either way.
    debug_assert!(
        write_compact
            || cfg!(all(
                target_arch = "x86_64",
        ...
        "compact-store elision is x86-only (raw-K gate)"
    );
    ...
    // GFNI bit-matrix form of the fold table, built once per pass and shared
    // read-only by every chunk (`FLOCK_NO_ZC_GFNI=1` keeps the gather fold).
    #[cfg(all(
        target_arch = "x86_64",
        target_feature = "avx512f",
    ...
    let r2_mats = r2_gfni_enabled().then(|| build_uni_skip_fold_mats(table));
    crate::epool::run_hetero_chunks(hi_size, |x_hi| {
        // SAFETY: the queue hands out each x_hi exactly once; chunk x_hi
        ...
        #[cfg(not(target_arch = "aarch64"))]
        {
            ...
            ))]
            {
                ...
                let mats_arg = r2_mats.as_ref();
                #[cfg(not(all(target_feature = "avx512vbmi", target_feature = "gfni")))]
                let mats_arg: Option<&[u64; 128]> = None;
                out = round2_lookahead_chunk_x86_avx512(
                    a_packed,
                    b_packed,
                    table,
                    mats_arg,
...
/// AVX-512 lookahead round-two chunk: the exact scalar schedule with the
/// four table folds batched through `fold_round2_pair_x86_unchecked_8` and
/// the per-group field products batched 4 lanes at a time through
/// `WideGhashX4`. Accumulation stays unreduced-XOR, which is linear, so any
/// grouping reduces to the same eight output values — bit-identical
/// transcript messages.
#[cfg(all(
    target_arch = "x86_64",
    target_feature = "avx512f",
...
/// Shared GFNI kill switch (`FLOCK_NO_ZC_GFNI=1` — see
/// [`crate::zerocheck::univariate_skip_optimized::ENV_NO_ZC_GFNI`]) for the
/// round-two batched fold. Output is bit-identical either way.
#[cfg(all(
    target_arch = "x86_64",
    target_feature = "avx512f",
    target_feature = "avx512vbmi",
...
fn r2_gfni_enabled() -> bool {
    static ON: std::sync::OnceLock<bool> = std::sync::OnceLock::new();
    *ON.get_or_init(|| {
        std::env::var_os(crate::zerocheck::univariate_skip_optimized::ENV_NO_ZC_GFNI).as_deref()
            != Some(std::ffi::OsStr::new("1"))
    })
}

/// GFNI matrices for [`gfni_fold64_rows`]: the univariate-skip fold table's
/// eight 256-entry byte banks are F2-linear (XOR-composed from eight basis
/// weights, `T_j[0] = 0`), so bank `j` IS sixteen 8×8 bit matrices — one per
/// output byte of the folded F128. `mats[j * 16 + k]` holds output byte `k`
/// of bank `j`, in `VGF2P8AFFINEQB` encoding (`out.bit[i] =
/// parity(byte[7-i] & in)`). 1 KiB per table instead of 32 KiB.
#[cfg(all(
    target_arch = "x86_64",
    target_feature = "avx512f",
    target_feature = "avx512vbmi",
...
fn build_uni_skip_fold_mats(table: &UniSkipFoldTable) -> [u64; 128] {
    debug_assert_eq!(table.n_chunks, 8);
    build_row_fold_mats(&table.data)
}
...
fn build_row_fold_mats(data: &[F128]) -> [u64; 128] {
...
unsafe fn gfni_fold64_rows(rows: *const u8, mats: &[u64; 128], out: *mut F128) {
...
#[allow(clippy::too_many_arguments)]
fn round2_lookahead_chunk_x86_avx512(
    a_packed: &[u8],
    b_packed: &[u8],
    table: &UniSkipFoldTable,
    mats: Option<&[u64; 128]>,
    ...
    use crate::field::gf2_128::x86_64::{WideGhashX4, f128x4_set, ghash_mul_x4};

    let n_chunks = table.n_chunks;
    debug_assert_eq!(n_chunks, 8);
    let table_ptr = table.data.as_ptr();
    // GFNI batch fold: 32 consecutive pairs = 64 consecutive rows per side,
    // prefolded in one bit-matrix batch (padded pairs fold garbage that the
    // consume path below overwrites with zeros exactly as before). The cfg!
    // guard keeps `use_batch` false on builds where the refill block below
    // compiles out.
    ...
    // SAFETY: avx512f/vpclmulqdq are cfg-guaranteed; every row slice below is
    // bounds-checked before its pointer is taken, and the fold kernels read
    // exactly their 8-byte rows (64 rows per batched block) within bounds.
    unsafe {
        let mut acc1 = WideGhashX4::zero(); // (p1_even, pinf_even, p1_odd, pinf_odd)
        let mut acc2 = WideGhashX4::zero(); // (w0, w1, w2, w3)
        ...
        let mut fold_pair = |x_lo: usize,
         ...
         -> Option<[F128; 4]> {
            ...
            if use_batch && x_lo.is_multiple_of(32) {
                // Refill the 64-row fold caches for pairs x_lo..x_lo+32.
                // Every block's rows lie inside the chunk's row range, so the
                // batched loads read exactly the bytes the pair path reads.
                let g0 = row_base + 2 * x_lo;
                debug_assert!((g0 + 64) * n_chunks <= a_packed.len());
                debug_assert!((g0 + 64) * n_chunks <= b_packed.len());
                let m = mats.unwrap();
                gfni_fold64_rows(a_packed.as_ptr().add(g0 * n_chunks), m, fa.as_mut_ptr());
                gfni_fold64_rows(b_packed.as_ptr().add(g0 * n_chunks), m, fb.as_mut_ptr());
            }
            if ((pair_idx_base + x_lo) & pair_in_block_mask) >= useful_pairs_inclusive {
                if write_compact {
                    anchors[2 * x_lo] = F128::ZERO;
        ...
        for u in 0..lo_size / 2 {
            ...
            // (a0w, a1w, a2w, a3w) in one 4-lane GHASH batch, then back to
            // scalars for the cheap XOR assembly of the product operands.
            let aw_v = ghash_mul_x4(
                f128x4_set(a0, a1, a2, a3),
                f128x4_set(wt, wt, wt, wt),
            );
            let mut aw = [F128::ZERO; 4];
            core::arch::x86_64::_mm512_storeu_si512(
                aw.as_mut_ptr().cast::<core::arch::x86_64::__m512i>(),
                aw_v,
            );
            let [a0w, a1w, a2w, a3w] = aw;

            // Lanes: (p1_even, pinf_even, p1_odd, pinf_odd). Padded sides are
            // all-zero, so their lanes accumulate zero — identical to the
            // scalar skip.
            acc1.mul_acc(
                f128x4_set(a1w, a0w + a1w, a3w, a2w + a3w),
                f128x4_set(b1, b0 + b1, b3, b2 + b3),
            );
            let (e_aw, e_b) = (a0w + a2w, b0 + b2);
            let (o_aw, o_b) = (a1w + a3w, b1 + b3);
            acc2.mul_acc(
                f128x4_set(a2w, e_aw, o_aw, e_aw + o_aw),
                f128x4_set(b2, e_b, o_b, e_b + o_b),
            );
        }

        let l1 = acc1.fold_lanes();
        let l2 = acc2.fold_lanes();
        [
            l1[0].reduce(),
            l1[1].reduce(),
            l1[2].reduce(),
...
pub(crate) fn fold2_compact_and_round45_into(
    ...
    fold2_compact_and_round45_into_impl(compact, table, None, rho1, rho2, r_next4, a_out, b_out)
}

/// Raw-input K pass: identical messages and outputs, but the composed tables
/// are folded straight from the ORIGINAL packed rows instead of the compact
/// anchor+delta state. Expanding the compact algebra gives, per group `g`,
///
///   `a_out[g] = Σ_c s_c · F_z(row_{4g+c})`,
///   `(s_0, s_1, s_2, s_3) = ((1+ρ₁)(1+ρ₂), ρ₁(1+ρ₂), ρ₂(1+ρ₁), ρ₁ρ₂)`
///
/// — the plain two-level tensor fold of four consecutive rows, exact field
/// algebra, so the outputs are the same field elements bit-for-bit. With the
/// GFNI row fold this needs no compact state at all: round two skips its
/// 1.5 GiB of anchor/delta stores and this pass reads the 1 GiB of original
/// rows instead of 1.5 GiB of compact state. Callers gate on
/// [`zc_raw_k_supported`]; `compact` is still passed for its recorded length
...
#[allow(clippy::too_many_arguments)]
pub(crate) fn fold2_raw_and_round45_into(
    compact: &UniSkipCompactFold,
    a_packed: &[u8],
    b_packed: &[u8],
    table: &UniSkipFoldTable,
    ...
    fold2_compact_and_round45_into_impl(
        compact,
        table,
...
/// Whether the raw-input K pass (and round two's matching compact-store
/// elision) is available for this shape: the GFNI build, the shared GFNI
/// kill switch, an extra `FLOCK_NO_ZC_RAWK=1` rollback, and a group count
/// whose per-chunk output is a multiple of the 64-group batch.
pub(crate) fn zc_raw_k_supported(n_groups: usize) -> bool {
    #[cfg(all(
        target_arch = "x86_64",
        target_feature = "avx512f",
        target_feature = "avx512vbmi",
    ...
    {
        if !r2_gfni_enabled() || std::env::var_os("FLOCK_NO_ZC_RAWK").is_some() {
            return false;
        }
        if !(n_groups >= 4 && n_groups.is_power_of_two()) {
            return false;
        }
        let n_vars = n_groups.trailing_zeros() as usize - 1;
        let n_hi = cascade_k_pass_n_hi(n_vars);
        let lo_size = n_groups >> (n_hi + 1);
        let out_chunk = 2 * lo_size;
        out_chunk >= 64 && out_chunk.is_multiple_of(64)
    }
    #[cfg(not(all(
        target_arch = "x86_64",
        target_feature = "avx512f",
...
#[allow(clippy::too_many_arguments)]
fn fold2_compact_and_round45_into_impl(
    compact: &UniSkipCompactFold,
    table: &UniSkipFoldTable,
    raw: Option<(&[u8], &[u8])>,
    rho1: F128,
    ...
    if let Some((a_packed, b_packed)) = raw {
    ...
    let partials_base = crate::epool::SyncPtr(partials.as_mut_ptr());
    let la_base = crate::epool::SyncPtr(la_partials.as_mut_ptr());
    // GFNI bit-matrix forms of both λ-scaled tables, built once per pass and
    // shared read-only by every chunk (`FLOCK_NO_ZC_GFNI=1` keeps gathers).
    #[cfg(all(
        target_arch = "x86_64",
        target_feature = "avx512f",
        target_feature = "avx512vbmi",
    ...
    let k_mats = (r2_gfni_enabled() && std::env::var_os("FLOCK_NO_K_GFNI").is_none())
        .then(|| (build_row_fold_mats(&table_l1), build_row_fold_mats(&table_l3)));
    // Raw-input mode: the four tensor-fold scale classes as GFNI matrix
    // blocks (see `fold2_raw_and_round45_into` — scaling the basis scales
    // every composed table entry exactly).
    #[cfg(all(
        target_arch = "x86_64",
        target_feature = "avx512f",
        target_feature = "avx512vbmi",
    ...
    let raw_mats = raw.map(|_| {
        let scales = [
            (F128::ONE + rho1) * (F128::ONE + rho2),
            rho1 * (F128::ONE + rho2),
            rho2 * (F128::ONE + rho1),
            rho1 * rho2,
        ];
        let mats: [[u64; 128]; 4] = std::array::from_fn(|c| {
            // Only the 64 one-hot basis entries feed the matrices; scaling
            // them scales every composed entry exactly (F2-bilinearity).
            let mut scaled = vec![F128::ZERO; 8 * 256];
            for j in 0..8 {
                for bit in 0..8 {
                    let idx = j * 256 + (1 << bit);
                    scaled[idx] = table.data[idx] * scales[c];
                }
            }
            build_row_fold_mats(&scaled)
        });
        mats
    });
    #[cfg(not(all(
    ...
    assert!(
        raw.is_none(),
        "raw K pass requires the GFNI build (gate on zc_raw_k_supported)"
    );
    crate::epool::run_hetero_chunks(hi_size, |x_hi| {
        // SAFETY: exclusive per-chunk ownership; the queue join publishes the
        // writes before the reduction below reads them.
        ...
        ))]
        {
            ...
            let b_out = unsafe { std::slice::from_raw_parts_mut(b_ptr, out_chunk) };
            #[cfg(all(target_feature = "avx512vbmi", target_feature = "gfni"))]
            let mats_arg = k_mats.as_ref().map(|(l1, l3)| (l1, l3));
            #[cfg(not(all(target_feature = "avx512vbmi", target_feature = "gfni")))]
            let mats_arg: Option<(&[u64; 128], &[u64; 128])> = None;
            #[cfg(all(target_feature = "avx512vbmi", target_feature = "gfni"))]
            let raw_arg = match (raw, raw_mats.as_ref()) {
                (Some((ap, bp)), Some(mats4)) => Some((ap, bp, mats4)),
                _ => None,
            };
            #[cfg(not(all(target_feature = "avx512vbmi", target_feature = "gfni")))]
            let raw_arg: Option<(&[u8], &[u8], &[[u64; 128]; 4])> = None;
            outv = fold2_round45_chunk_x86_avx512(
                compact, table, &table_l1, &table_l3, mats_arg, raw_arg, rho2, a_out, b_out,
                eq_lo, lo_size, x_hi,
            );
        }
        #[cfg(not(any(
...
/// AVX-512 cascade K chunk: the scalar schedule with the 32 delta-table
/// lookups per two groups run as four independent XMM XOR chains, the anchor
/// folds and the message products batched four lanes at a time through
/// GHASH. Unreduced-XOR accumulation is linear, so the regrouped sums reduce
/// to exactly the scalar outputs.
#[cfg(all(
    target_arch = "x86_64",
    target_feature = "avx512f",
    target_feature = "vpclmulqdq"
...
#[allow(clippy::too_many_arguments)]
#[allow(clippy::too_many_arguments)]
fn fold2_round45_chunk_x86_avx512(
    compact: &UniSkipCompactFold,
    table: &UniSkipFoldTable,
    table_l1: &[F128],
    table_l3: &[F128],
    mats: Option<(&[u64; 128], &[u64; 128])>,
    raw: Option<(&[u8], &[u8], &[[u64; 128]; 4])>,
    rho2: F128,
    a_out: &mut [F128],
    b_out: &mut [F128],
    eq_lo: &[F128],
    ...
    let nc = table.n_chunks;
    debug_assert_eq!(nc, 8);
    let out_chunk = a_out.len();
    debug_assert!(out_chunk.is_multiple_of(2));
    let l1p = table_l1.as_ptr();
    let l3p = table_l3.as_ptr();
    // Raw-input fill (see `fold2_raw_and_round45_into`): the whole chunk's
    // outputs come straight from the original packed rows in 64-group
    // batches, then the identical message sweep below runs unchanged.
    #[cfg(all(target_feature = "avx512vbmi", target_feature = "gfni"))]
    if let Some((a_packed, b_packed, mats4)) = raw {
        assert!(
            out_chunk >= 64 && out_chunk.is_multiple_of(64),
            "raw K fill requires 64-group chunks (gate on zc_raw_k_supported)"
        );
        // SAFETY: the caller sized a_packed/b_packed to exactly 4 rows per
        // group; every batched load below stays inside the chunk's rows.
        unsafe {
            let g_base = x_hi * out_chunk;
            debug_assert!((4 * (g_base + out_chunk)) * nc <= a_packed.len());
            let mut blk = 0;
            while blk < out_chunk {
                gfni_fold256_rows_4scale(
                    a_packed.as_ptr().add(4 * (g_base + blk) * nc),
                    mats4,
                    a_out[blk..blk + 64].as_mut_ptr(),
                );
                gfni_fold256_rows_4scale(
                    b_packed.as_ptr().add(4 * (g_base + blk) * nc),
                    mats4,
                    b_out[blk..blk + 64].as_mut_ptr(),
                );
                blk += 64;
            }
        }
        // SAFETY: same feature set; the sweep reads only the freshly filled
        // outputs and the eq tables.
        return unsafe {
            fold2_round45_message_sweep_x86(a_out, b_out, eq_lo, lo_size)
        };
    }
    // GFNI batch fold: 32 consecutive groups = 128 consecutive delta rows,
    // split by their period-4 L1/L3 pattern into two 64-row bit-matrix
    // batches. The cfg! guard keeps `use_batch` false on builds where the
    // refill block below compiles out.
    let use_batch = cfg!(all(target_feature = "avx512vbmi", target_feature = "gfni"))
        && mats.is_some()
        && out_chunk >= 32
        && out_chunk.is_multiple_of(32);

    // SAFETY: features cfg-guaranteed; anchor/delta slices are bounds-checked
    // below and every table index is a u8 into a 8*256-entry bank.
    unsafe {
        let _ = raw;
        let rho2_v = f128x4_set(rho2, rho2, rho2, rho2);
        let mut f_l1 = [F128::ZERO; 64];
        let mut f_l3 = [F128::ZERO; 64];
        let mut g_local = 0;
        while g_local + 2 <= out_chunk {
            let g = x_hi * out_chunk + g_local;
            #[cfg(all(target_feature = "avx512vbmi", target_feature = "gfni"))]
            if use_batch && g_local.is_multiple_of(32) {
                // Refill both 64-row fold caches for groups g..g+32: delta
                // rows 4g..4g+128 (1 KiB) are [aL1, bL1, aL3, bL3] per group,
                // so qword-permutes peel the L1 pairs and L3 pairs into
                // register batches (row 2t+{0,1} of a batch = group g+t's
                // a/b rows). Reads exactly the bytes the gather path reads.
                let (m_l1, m_l3) = mats.unwrap();
                debug_assert!(32 * g + 1024 <= compact.deltas.len());
                let dp = compact.deltas.as_ptr().add(32 * g);
                let sel_l1 = _mm512_setr_epi64(0, 1, 4, 5, 8, 9, 12, 13);
                let sel_l3 = _mm512_setr_epi64(2, 3, 6, 7, 10, 11, 14, 15);
                let mut z_l1 = [_mm512_setzero_si512(); 8];
                let mut z_l3 = [_mm512_setzero_si512(); 8];
                for k in 0..8 {
                    let z_e = _mm512_loadu_si512(dp.add(128 * k) as *const __m512i);
                    let z_o = _mm512_loadu_si512(dp.add(128 * k + 64) as *const __m512i);
                    z_l1[k] = _mm512_permutex2var_epi64(z_e, sel_l1, z_o);
                    z_l3[k] = _mm512_permutex2var_epi64(z_e, sel_l3, z_o);
                }
                gfni_fold64_regs(z_l1, m_l1, f_l1.as_mut_ptr());
                gfni_fold64_regs(z_l3, m_l3, f_l3.as_mut_ptr());
            }
            let anc = &compact.anchors[4 * g..4 * g + 8];
            let d = &compact.deltas[32 * g..32 * g + 64];
            let folded = ghash_mul_x4(
                f128x4_set(
                    anc[0] + anc[2],
                    anc[1] + anc[3],
                    anc[4] + anc[6],
            ...
            let mut base4 = [F128::ZERO; 4];
            _mm512_storeu_si512(base4.as_mut_ptr().cast::<__m512i>(), folded);
            let init = [
                anc[0] + base4[0],
                anc[1] + base4[1],
                anc[4] + base4[2],
            ...
            let mut acc = [
                _mm_set_epi64x(init[0].hi as i64, init[0].lo as i64),
                _mm_set_epi64x(init[1].hi as i64, init[1].lo as i64),
                _mm_set_epi64x(init[2].hi as i64, init[2].lo as i64),
                _mm_set_epi64x(init[3].hi as i64, init[3].lo as i64),
            ];
            if use_batch {
                let gb = g_local % 32;
                for (slot, r) in [(0usize, 2 * gb), (1, 2 * gb + 1), (2, 2 * gb + 2), (3, 2 * gb + 3)]
                {
                    acc[slot] = _mm_xor_si128(
                        acc[slot],
                        _mm_xor_si128(
                            _mm_loadu_si128((f_l1.as_ptr().add(r)).cast::<__m128i>()),
                            _mm_loadu_si128((f_l3.as_ptr().add(r)).cast::<__m128i>()),
                        ),
                    );
                }
            } else {
                for j in 0..8 {
                    let l1b = l1p.add(j * 256);
                    let l3b = l3p.add(j * 256);
                    acc[0] = _mm_xor_si128(
                        acc[0],
                        _mm_load_si128(l1b.add(d[j] as usize).cast::<__m128i>()),
                    );
                    acc[1] = _mm_xor_si128(
                        acc[1],
                        _mm_load_si128(l1b.add(d[8 + j] as usize).cast::<__m128i>()),
                    );
                    acc[2] = _mm_xor_si128(
                        acc[2],
                        _mm_load_si128(l1b.add(d[32 + j] as usize).cast::<__m128i>()),
                    );
                    acc[3] = _mm_xor_si128(
                        acc[3],
                        _mm_load_si128(l1b.add(d[40 + j] as usize).cast::<__m128i>()),
                    );
                    acc[0] = _mm_xor_si128(
                        acc[0],
                        _mm_load_si128(l3b.add(d[16 + j] as usize).cast::<__m128i>()),
                    );
                    acc[1] = _mm_xor_si128(
                        acc[1],
                        _mm_load_si128(l3b.add(d[24 + j] as usize).cast::<__m128i>()),
                    );
                    acc[2] = _mm_xor_si128(
                        acc[2],
                        _mm_load_si128(l3b.add(d[48 + j] as usize).cast::<__m128i>()),
                    );
                    acc[3] = _mm_xor_si128(
                        acc[3],
                        _mm_load_si128(l3b.add(d[56 + j] as usize).cast::<__m128i>()),
                    );
                }
            }
            a_out[g_local] = core::mem::transmute::<__m128i, F128>(acc[0]);
            b_out[g_local] = core::mem::transmute::<__m128i, F128>(acc[1]);
            a_out[g_local + 1] = core::mem::transmute::<__m128i, F128>(acc[2]);
            b_out[g_local + 1] = core::mem::transmute::<__m128i, F128>(acc[3]);
            g_local += 2;
        }

        // Message sweep — same batching as the round-two lookahead kernel.
        fold2_round45_message_sweep_x86(a_out, b_out, eq_lo, lo_size)
    }
}

/// The K chunk's message sweep, shared by the compact and raw fill arms:
/// identical products and slot order over the freshly composed outputs.
#[cfg(all(
    target_arch = "x86_64",
    target_feature = "avx512f",
    target_feature = "vpclmulqdq"
...
unsafe fn fold2_round45_message_sweep_x86(
    a_out: &[F128],
    b_out: &[F128],
    eq_lo: &[F128],
    lo_size: usize,
    ...
    use crate::field::gf2_128::x86_64::{WideGhashX4, f128x4_set, ghash_mul_x4};
    use core::arch::x86_64::*;
    // SAFETY: pure register/product work over in-bounds slices.
    unsafe {
        let mut acc1 = WideGhashX4::zero(); // (p1_even, pinf_even, p1_odd, pinf_odd)
        let mut acc2 = WideGhashX4::zero(); // (w0, w1, w2, w3)
        for t in 0..lo_size / 2 {
            ...
            let wt = eq_lo[2 * t + 1];
            let aw_v = ghash_mul_x4(f128x4_set(a0, a1, a2, a3), f128x4_set(wt, wt, wt, wt));
            let mut aw = [F128::ZERO; 4];
            _mm512_storeu_si512(aw.as_mut_ptr().cast::<__m512i>(), aw_v);
            let [a0w, a1w, a2w, a3w] = aw;
            acc1.mul_acc(
                f128x4_set(a1w, a0w + a1w, a3w, a2w + a3w),
                f128x4_set(b1, b0 + b1, b3, b2 + b3),
...
/// Fold 256 consecutive packed rows through four scale-class matrix blocks
/// (row `4t + c` takes class `c`) and write the 64 class-summed outputs:
/// `out[t] = Σ_c F_{s_c}(row_{4t+c})` — the raw K pass's composed output.
/// The period-4 class peel is three `vpermt2q` per class per 32 rows; the
/// per-class folds reuse [`gfni_fold64_regs`].
#[cfg(all(
    target_arch = "x86_64",
    target_feature = "avx512f",
    target_feature = "avx512vbmi",
...
#[target_feature(enable = "avx512f,avx512vbmi,gfni")]
unsafe fn gfni_fold256_rows_4scale(rows: *const u8, mats4: &[[u64; 128]; 4], out: *mut F128) {
    use core::arch::x86_64::*;
    // SAFETY (whole body): caller guarantees 2048 readable bytes at `rows`
    // and 64 writable F128s at `out`; all shuffle indices are in range.
    unsafe {
        let combine = _mm512_setr_epi64(0, 1, 2, 3, 8, 9, 10, 11);
        let mut folded = [[F128::ZERO; 64]; 4];
        for (c, class_out) in folded.iter_mut().enumerate() {
            let sel = _mm512_setr_epi64(
                c as i64,
                c as i64 + 4,
                c as i64 + 8,
                c as i64 + 12,
            ...
            let mut cls = [_mm512_setzero_si512(); 8];
            for (q, slot) in cls.iter_mut().enumerate() {
                let z0 = _mm512_loadu_si512(rows.add(256 * q) as *const __m512i);
                let z1 = _mm512_loadu_si512(rows.add(256 * q + 64) as *const __m512i);
                let z2 = _mm512_loadu_si512(rows.add(256 * q + 128) as *const __m512i);
                let z3 = _mm512_loadu_si512(rows.add(256 * q + 192) as *const __m512i);
                let t01 = _mm512_permutex2var_epi64(z0, sel, z1);
                let t23 = _mm512_permutex2var_epi64(z2, sel, z3);
                *slot = _mm512_permutex2var_epi64(t01, combine, t23);
            }
            gfni_fold64_regs(cls, &mats4[c], class_out.as_mut_ptr());
        }
        for t in 0..16 {
            let f0 = _mm512_loadu_si512(folded[0].as_ptr().add(4 * t) as *const __m512i);
            let f1 = _mm512_loadu_si512(folded[1].as_ptr().add(4 * t) as *const __m512i);
            let f2 = _mm512_loadu_si512(folded[2].as_ptr().add(4 * t) as *const __m512i);
            let f3 = _mm512_loadu_si512(folded[3].as_ptr().add(4 * t) as *const __m512i);
            let v = _mm512_ternarylogic_epi64::<0x96>(f0, f1, f2);
            _mm512_storeu_si512(
                (out as *mut u8).add(64 * t) as *mut __m512i,
                _mm512_xor_si512(v, f3),
            );
        }
    }
}
...
/// AVX-512 chunk for the cascade rounds-7 lookahead pass: the eight fold4s
/// per group run as two `fold4_nested`-style ZMM calls (four outputs per
/// call, mids kept in registers) and the message products batch through
/// two four-lane GHASH accumulators. Unreduced-XOR regrouping is linear, so
/// the outputs are bit-identical to the scalar chunk.
#[cfg(all(
    target_arch = "x86_64",
    target_feature = "avx512f",
    target_feature = "vpclmulqdq"
...
#[allow(clippy::too_many_arguments)]
fn fold2_round67_chunk_x86_avx512(
    a_in: &[F128],
    b_in: &[F128],
    a_out: &mut [F128],
    b_out: &mut [F128],
    ...
    /// Four nested pair-folds (16 source F128 -> 4 outputs) with the r0 mids
    /// kept in ZMM — the exact `t0 + r1*(t0+t1)` over `v + r0*(v+v')` chain.
    #[inline(always)]
    unsafe fn fold4_x4(
        src: *const F128,
        r0: __m512i,
        ...
        unsafe {
            let v0 = _mm512_loadu_si512(src.cast::<__m512i>());
            let v1 = _mm512_loadu_si512(src.add(4).cast::<__m512i>());
            let v2 = _mm512_loadu_si512(src.add(8).cast::<__m512i>());
            let v3 = _mm512_loadu_si512(src.add(12).cast::<__m512i>());
            let e01 = _mm512_permutex2var_epi64(v0, idx_even, v1);
            let o01 = _mm512_permutex2var_epi64(v0, idx_odd, v1);
            let mid01 = _mm512_xor_si512(e01, ghash_mul_x4(r0, _mm512_xor_si512(e01, o01)));
            let e23 = _mm512_permutex2var_epi64(v2, idx_even, v3);
            let o23 = _mm512_permutex2var_epi64(v2, idx_odd, v3);
            let mid23 = _mm512_xor_si512(e23, ghash_mul_x4(r0, _mm512_xor_si512(e23, o23)));
            let low = _mm512_permutex2var_epi64(mid01, idx_even, mid23);
            let high = _mm512_permutex2var_epi64(mid01, idx_odd, mid23);
            _mm512_xor_si512(low, ghash_mul_x4(r1, _mm512_xor_si512(low, high)))
        }
    }

    // SAFETY: features cfg-guaranteed; chunk geometry supplies sixteen
    // inputs per four outputs (bounds asserted by the caller's chunking).
    unsafe {
        let r0 = _mm512_broadcast_i32x4(_mm_set_epi64x(rho3.hi as i64, rho3.lo as i64));
        let r1 = _mm512_broadcast_i32x4(_mm_set_epi64x(rho4.hi as i64, rho4.lo as i64));
        ...
        for t in 0..eq_lo.len() / 2 {
            let i = 16 * t;
            let o = 4 * t;
            let av = fold4_x4(a_in.as_ptr().add(i), r0, r1, idx_even, idx_odd);
            let bv = fold4_x4(b_in.as_ptr().add(i), r0, r1, idx_even, idx_odd);
            _mm512_storeu_si512(a_out.as_mut_ptr().add(o).cast::<__m512i>(), av);
            _mm512_storeu_si512(b_out.as_mut_ptr().add(o).cast::<__m512i>(), bv);
            let mut aa = [F128::ZERO; 4];
            let mut bb = [F128::ZERO; 4];
            _mm512_storeu_si512(aa.as_mut_ptr().cast::<__m512i>(), av);
            _mm512_storeu_si512(bb.as_mut_ptr().cast::<__m512i>(), bv);
            let [a0, a1, a2, a3] = aa;
            let [b0, b1, b2, b3] = bb;
            let wt = eq_lo[2 * t + 1];
            let aw_v = ghash_mul_x4(f128x4_set(a0, a1, a2, a3), f128x4_set(wt, wt, wt, wt));
...
pub(crate) fn fold2_plain_and_round67_into(
    ...
    let chunk_partial =
        |a_in: &[F128], b_in: &[F128], a_out: &mut [F128], b_out: &mut [F128]| -> [F128; 8] {
            ...
            {
                fold2_round67_chunk_x86_avx512(a_in, b_in, a_out, b_out, rho3, rho4, eq_lo)
            }
            #[cfg(not(any(
                target_arch = "aarch64",
                all(
...
mod tests {
    ...
    /// The GFNI batched row fold must reproduce `fold_one_row` exactly for
    /// every row: same table, random rows, byte-for-byte equality after the
    /// full transpose→GFNI→inverse-transpose pipeline.
    #[cfg(all(
        target_arch = "x86_64",
        target_feature = "avx512f",
        target_feature = "avx512vbmi",
    ...
    #[test]
    fn gfni_fold64_rows_matches_fold_one_row() {
        let z = F128::new(0x0123_4567_89ab_cdef, 0x0fed_cba9_8765_4321);
        let table = UniSkipFoldTable::new(6, z);
        let mats = build_uni_skip_fold_mats(&table);

        let mut state = 0x243f_6a88_85a3_08d3u64;
        let mut rows = [0u8; 64 * 8];
        ...
        let mut out = [F128::ZERO; 64];
        // SAFETY: 512 readable bytes, 64 writable F128s, features cfg-gated.
        unsafe { gfni_fold64_rows(rows.as_ptr(), &mats, out.as_mut_ptr()) };
        for r in 0..64 {
            let expected = table.fold_one_row(&rows[r * 8..(r + 1) * 8]);
            assert_eq!(out[r], expected, "row {r}");
        }
    }

    /// The K-pass GFNI batch arm (L1/L3 split permutation + two 64-row
    /// batches) must reproduce the gather arm byte-for-byte: same compact
    /// state, both arms of `fold2_round45_chunk_x86_avx512`, identical
    /// outputs and message slots. Exercises a shape (out_chunk = 64) big
    /// enough to engage the batch, which the small end-to-end shapes miss.
    #[cfg(all(
        target_arch = "x86_64",
        target_feature = "avx512f",
        target_feature = "avx512vbmi",
    ...
    #[test]
    fn fold2_round45_gfni_batch_matches_gather_arm() {
        let z = F128::new(0x1357_9bdf_2468_ace0, 0x0ace_2468_bdf1_3579);
        let table = UniSkipFoldTable::new(6, z);
        let lambda1 = F128::new(0x1111_2222_3333_4444, 0x5555_6666_7777_8888);
        let lambda3 = F128::new(0x9999_aaaa_bbbb_cccc, 0xdddd_eeee_ffff_0001);
        let table_l1 = table.scaled_linear(lambda1);
        let table_l3 = table.scaled_linear(lambda3);
        let mats_l1 = build_row_fold_mats(&table_l1);
        let mats_l3 = build_row_fold_mats(&table_l3);

        let out_chunk = 64usize;
        let lo_size = out_chunk / 2;
        let n_pairs = 2 * out_chunk;
        ...
        let anchors: Vec<F128> = (0..2 * n_pairs).map(|_| F128::new(next(), next())).collect();
        let mut deltas = crate::scratch::ScratchBytes::take(2 * n_pairs * 8);
        for byte in deltas.iter_mut() {
            *byte = (next() >> 33) as u8;
        }
        let compact = UniSkipCompactFold { anchors, deltas };
        let rho2 = F128::new(0x0f0f_1e1e_2d2d_3c3c, 0x4b4b_5a5a_6969_7878);
        let eq_lo: Vec<F128> = (0..lo_size).map(|_| F128::new(next(), next())).collect();

        let mut a_ref = vec![F128::ZERO; out_chunk];
        let mut b_ref = vec![F128::ZERO; out_chunk];
        let out_ref = fold2_round45_chunk_x86_avx512(
            &compact, &table, &table_l1, &table_l3, None, None, rho2, &mut a_ref, &mut b_ref,
            &eq_lo, lo_size, 0,
        );
        let mut a_gfni = vec![F128::ZERO; out_chunk];
        let mut b_gfni = vec![F128::ZERO; out_chunk];
        let out_gfni = fold2_round45_chunk_x86_avx512(
            &compact,
            &table,
            &table_l1,
            &table_l3,
            Some((&mats_l1, &mats_l3)),
            None,
            rho2,
            &mut a_gfni,
            &mut b_gfni,
        ...
        assert_eq!(a_ref, a_gfni);
        assert_eq!(b_ref, b_gfni);
        assert_eq!(out_ref, out_gfni);
    }

    /// The raw-input K pass must reproduce the compact route bit-for-bit at
    /// a shape that engages the 64-group batch (m = 25 → out_chunk = 64),
    /// including ranked-style padded blocks: the compact route consumes
    /// pre-zeroed anchors/deltas for padded pairs while the raw route folds
    /// the (genuinely zero) padded rows — both must land on the same field
    /// elements, messages, and deferred round-five quadratic.
    #[cfg(all(
        target_arch = "x86_64",
        target_feature = "avx512f",
        target_feature = "avx512vbmi",
    ...
    #[test]
    fn raw_k_pass_matches_compact_route() {
        const K_SKIP: usize = 6;
        for (seed, ranked_padding) in [(0xC0FF_EE01u64, true), (0xC0FF_EE02, false)] {
            let f = la_fixture(25, seed, ranked_padding);
            let (compact, _m1, _mi, _la) = uni_skip_fold_and_round_pair_compact_padded_lookahead(
                &f.a_packed,
                &f.b_packed,
                f.m,
                K_SKIP,
            ...
            let n_groups = n_pairs / 2;
            assert!(
                zc_raw_k_supported(n_groups),
                "test shape must engage the raw K batch"
            );
            let rho1 = f.rho_grid[3];
            let rho2 = f.mlv[2];
            let mut r_next4 = vec![F128::ONE; n_groups.trailing_zeros() as usize];
            let tail_len = r_next4.len() - 1;
            r_next4[1..].copy_from_slice(&f.mlv[3..3 + tail_len]);
            assert_ne!(r_next4[1], F128::ZERO);

            let mut a_c = vec![F128::ZERO; n_groups];
            let mut b_c = vec![F128::ZERO; n_groups];
            let (c1, ci, cla) = fold2_compact_and_round45_into(
                &compact, &f.table, rho1, rho2, &r_next4, &mut a_c, &mut b_c,
            );
            let mut a_r = vec![F128::ZERO; n_groups];
            let mut b_r = vec![F128::ZERO; n_groups];
            let (r1m, rim, rla) = fold2_raw_and_round45_into(
                &compact,
                &f.a_packed,
                &f.b_packed,
                &f.table,
            ...
            assert_eq!(a_c, a_r, "composed a, ranked_padding={ranked_padding}");
            assert_eq!(b_c, b_r, "composed b, ranked_padding={ranked_padding}");
            assert_eq!((c1, ci), (r1m, rim), "round-four message");
            assert_eq!(cla.c, rla.c, "deferred round-five quadratic");
        }
    }
