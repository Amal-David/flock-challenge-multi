...
// ---------------------------------------------------------------------------
// C-lift bit matrices: for each polynomial-basis bit b, the map
// v -> x^b * phi_8(v) is F2-linear from one input byte to one F128 lane.
// Sixteen GFNI matrices encode its sixteen output bytes. The ranked C lift
// applies all 128 maps to 64 lanes once per proof, replacing 8,192 reduced
// GHASH products with byte-linear affine operations plus one plane transpose.
// ---------------------------------------------------------------------------
...
const NTT_EXT_GFNI_MATS: usize = 128 * 16;
...
static NTT_EXT_GFNI_MATS_CACHE: OnceLock<Vec<u64>> = OnceLock::new();
...
fn build_ntt_ext_gfni_mats() -> Vec<u64> {
    let mut mats = vec![0u64; NTT_EXT_GFNI_MATS];
    let mut gamma = F128::ONE;
    for b in 0..128 {
        let basis: [F128; 8] = std::array::from_fn(|j| gamma * PHI_8_TABLE[1 << j]);
        for k in 0..16 {
            let mut qword = 0u64;
            for i in 0..8 {
                let bit_index = 8 * k + i;
                ...
                for (j, value) in basis.iter().enumerate() {
                    let bit = if bit_index < 64 {
                        (value.lo >> bit_index) & 1
                    } else {
                        (value.hi >> (bit_index - 64)) & 1
                    };
                    row_bits |= (bit as u8) << j;
                }
                // VGF2P8AFFINEQB evaluates output bit i from matrix byte 7-i.
                qword |= (row_bits as u64) << (8 * (7 - i));
            }
            mats[b * 16 + k] = qword;
        }
...
pub(super) fn ntt_extend_f128_gfni_enabled() -> bool {
    static ON: OnceLock<bool> = OnceLock::new();
    *ON.get_or_init(|| std::env::var_os("FLOCK_NO_ZC_NTT_EXT_GFNI").is_none())
}
...
pub(super) fn ntt_extend_f128_vec_gfni(
    ...
    let mats = NTT_EXT_GFNI_MATS_CACHE.get_or_init(build_ntt_ext_gfni_mats);
    kernels::ntt_extend_f128_vec_gfni(in_s, inv_table, mats)
}
