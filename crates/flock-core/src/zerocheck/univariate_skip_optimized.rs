...
/// Exact same-binary rollback for the ranked identity-C Fold8 collapse.
const ENV_NO_ZC_C_FOLD8_MEDIUM_HORNER: &str = "FLOCK_NO_ZC_C_FOLD8_MEDIUM_HORNER";

#[inline]
fn ranked_c_fold8_medium_horner_enabled() -> bool {
    static ON: std::sync::LazyLock<bool> = std::sync::LazyLock::new(|| {
        std::env::var_os(ENV_NO_ZC_C_FOLD8_MEDIUM_HORNER).as_deref()
...
fn collapse_ranked_c_fold8_medium_horner(fold8: &[F128], inner_tail: &[F128]) -> Vec<F128> {
    ...
    for small_bank in 0..4 {
        ...
        for q in (0..15).rev() {
            ...
            for (value, &coefficient) in acc
                .iter_mut()
                .zip(&fold8[base..base + n_packed])
            {
        ...
        crate::field::f128_slice::add_scaled(
            &mut out,
            &acc,
            small_eq[small_bank] * medium_scale,
        );
    }
    out
}
                ...
                for bank in 0..16 {
                    ...
                    if wide {
                        ...
                        for packed in 0..n_packed {
                            fold4[dst + packed] +=
                                retained_top_eq[high] * fold8[src + packed];
                        }
                    }
                }
            }
    ...
    fn identity_c_fold_matches_row_major_fold4_drain() {
        ...
        for (m, useful_bits) in [(22usize, 15_409usize), (24, 15_409), (22, 1usize << 14)] {
                ...
                && ranked_c_fold8_medium_horner_enabled()
            {
                ...
                assert!(
                    fold4_new.is_empty(),
                    "Horner path still materialised Fold4"
                );
            } else {
                assert_eq!(
                    quad_new, quad_ref,
