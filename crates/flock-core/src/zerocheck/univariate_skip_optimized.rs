...
/// GFNI drain kill switch: exactly `FLOCK_NO_ZC_GFNI=1` restores the
/// byte-table gather drain as a same-binary control. Output is bit-identical
/// either way (same XOR terms, reassociated).
pub const ENV_NO_ZC_GFNI: &str = "FLOCK_NO_ZC_GFNI";
...
fn ab_eq_fold_gfni_enabled() -> bool {
    ...
    *ON.get_or_init(|| {
        std::env::var_os(ENV_NO_ZC_GFNI).as_deref() != Some(std::ffi::OsStr::new("1"))
    })
}

/// GFNI bit-matrix form of [`build_ab_eq_fold_tables`]: the pre-scaled table
/// `T_w` is never expanded to 256 entries — each of its 16 sub-tables is
/// represented by sixteen 8×8 output-byte bit matrices, built directly from
/// the eight scaled basis entries (`T_w[bm][1<<j] = convert[bm*256 + (1<<j)]
/// · eq_top_scaled[w]`, everything else is their XOR span). 2 KiB per `w`
/// instead of 64 KiB, and no 131k-product table expansion. Matrix qword
/// encoding matches `VGF2P8AFFINEQB`: `out.bit[i] = parity(byte[7-i] & in)`.
#[cfg(all(
    target_arch = "x86_64",
    target_feature = "avx512f",
    target_feature = "vpclmulqdq",
...
std::thread_local! {
    /// Per-worker byte-plane banks for the GFNI drain (`2^s × 16 × ELL`
    /// bytes — the same 1 KiB per bank as the F128 form, reused across every
    /// band and prove like [`AB_EQ_FOLD_BANKS`]).
    static AB_EQ_FOLD_PLANES: std::cell::RefCell<Vec<u8>> =
        const { std::cell::RefCell::new(Vec::new()) };
}

/// GFNI twin of [`process_one_x_hi_with_precomputed_ab_fold4_ab_eq_folded`]:
/// identical sweep and bank selection, but the per-chunk gathers become
/// `VGF2P8AFFINEQB` bit-matrix products into byte-plane-major banks and the
/// 64 KiB per-`w` tables never exist. The band-end fold assembles each
/// bank's planes back into F128 lanes once, then applies the identical
/// `eq_bot`/`eq_hi` weighting — bit-identical output.
#[cfg(all(
    target_arch = "x86_64",
    target_feature = "avx512f",
    target_feature = "vpclmulqdq",
...
#[inline]
#[allow(clippy::too_many_arguments)]
fn process_one_x_hi_with_precomputed_ab_fold4_ab_eq_folded_gfni(
    x_hi: usize,
    big_lo_size: usize,
    n_lo_and_inner: usize,
    within_outer_mask: usize,
    ...
    const PLANE_BANK_BYTES: usize = 16 * ELL;
    debug_assert_eq!(plane_banks.len(), eq_bot.len() * PLANE_BANK_BYTES);
    debug_assert_eq!(eq_bot.len(), 1usize << bank_bits);
    plane_banks.fill(0);
    ...
    for x_outer_lo in 0..big_lo_size {
        ...
        let w = x_outer_lo >> bank_bits;
        let u = x_outer_lo & bank_mask;
        let mats_w: &[u64; 256] = (&mats[w * 256..(w + 1) * 256])
            .try_into()
            .expect("one 16×16 qword matrix block per w");
        let bank: &mut [u8; PLANE_BANK_BYTES] = (&mut plane_banks
            [u * PLANE_BANK_BYTES..(u + 1) * PLANE_BANK_BYTES])
            .try_into()
            .expect("one plane bank per low x_lo index");
    ...
    partial_ab.fill(F128::ZERO);
    for (bank, eq_bot_val) in plane_banks.chunks_exact(PLANE_BANK_BYTES).zip(eq_bot) {
        for (lane, out) in partial_ab.iter_mut().enumerate() {
            let mut lo = 0u64;
            let mut hi = 0u64;
            for k in 0..8 {
...
fn round1_shift_reduce_ab_packed_padded_with_precomputed_with_fold(
    ...
    match fold.bank_bits(eq.n_lo) {
        ...
        Some(bank_bits) => {
            ...
            // GFNI drain: the pre-scaled tables become 8×8 bit matrices and
            // the gathers become VGF2P8AFFINEQB products (`FLOCK_NO_ZC_GFNI=1`
            // restores the table drain; bit-identical output either way).
            #[cfg(all(
                target_arch = "x86_64",
                target_feature = "avx512f",
                target_feature = "vpclmulqdq",
            ...
            let use_gfni = ab_eq_fold_gfni_enabled();
            #[cfg(not(all(
                target_arch = "x86_64",
                target_feature = "avx512f",
                target_feature = "vpclmulqdq",
            ...
            if use_gfni {
                ...
                ))]
                {
                    ...
                    crate::epool::run_hetero_chunks(hi_size, |x_hi| {
                        let mut partial = [F128::ZERO; ELL];
                        AB_EQ_FOLD_PLANES.with(|cell| {
                            let mut banks = cell.borrow_mut();
                            if banks.len() != plane_len {
                                banks.clear();
                                banks.resize(plane_len, 0);
                            }
                            process_one_x_hi_with_precomputed_ab_fold4_ab_eq_folded_gfni(
                                x_hi,
                                big_lo_size,
                                n_lo_and_inner,
                                within_outer_mask,
                        ...
                        // SAFETY: each queue index owns one disjoint output
                        // slot and the synchronous queue join publishes all
                        // writes before reduction.
                        unsafe { *partials_base.ptr().add(x_hi) = partial };
                    });
                }
            } else {
                ...
                crate::epool::run_hetero_chunks(hi_size, |x_hi| {
                    ...
                    // SAFETY: each queue index owns one disjoint output slot
                    // and the synchronous queue join publishes all writes
                    // before reduction.
                    unsafe { *partials_base.ptr().add(x_hi) = partial };
                });
...
mod tests {
    ...
    /// The GFNI drain must reproduce the table drain's bank sums exactly:
    /// same convert basis, same `eq_top` scales, random rows, every
    /// `n_b_med` parity. Assembles the plane-major GFNI bank back into F128
    /// lanes and compares bit-for-bit against the gather kernel.
    #[cfg(all(
        target_arch = "x86_64",
        target_feature = "avx512f",
        target_feature = "vpclmulqdq",
    ...
    #[test]
    fn gfni_convert_fold_matches_table_gathers() {
        let convert = convert_table();
        let eq_top: Vec<F128> = (0..4)
            .map(|i| F128::new(0x0123_4567_89ab_cdef ^ (i as u64), 0x0fed_cba9_8765_4321 ^ ((i as u64) << 17)))
            .collect();
        let tables = build_ab_eq_fold_tables(&eq_top, convert);
        let mats = build_ab_eq_fold_mats(&eq_top, convert);

        let mut state = 0x243f_6a88_85a3_08d3u64;
        let mut rows = [[0u8; ELL]; 1 << N_MEDIUM];
        ...
        for w in 0..eq_top.len() {
            for n_b_med in [1usize, 2, 7, 15, 16] {
                let mut bank = [F128::ZERO; ELL];
                kernels::accumulate_convert_ab_nomul(
                    &rows,
                    n_b_med,
                    &tables[w * CONVERT_TABLE_SIZE..(w + 1) * CONVERT_TABLE_SIZE],
                ...
                kernels::accumulate_convert_ab_nomul_gfni(
                    &rows,
                    n_b_med,
                    (&mats[w * 256..(w + 1) * 256]).try_into().unwrap(),
                    &mut planes,
                );
                for lane in 0..ELL {
                    let mut lo = 0u64;
                    ...
                    for k in 8..16 {
                        hi |= (planes[k * ELL + lane] as u64) << (8 * (k - 8));
                    }
                    assert_eq!(
                        bank[lane],
                        F128 { lo, hi },
                        "w={w} n_b_med={n_b_med} lane={lane}"
                    );
                }
            }
        }
