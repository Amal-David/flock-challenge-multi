...
impl Round1AbWindowPlan {
    ...
    /// Publish an all-zero window with this plan's output-store class.
    ///
    /// The streaming BLAKE3 producer uses this for a medium window that lies
    /// wholly beyond the circuit's useful prefix.  Keeping the plan's
    /// temporal/XMM-stream/ZMM-stream choice matters: the scratch allocation
    /// is uninitialised, and replacing the regular transform must still write
    /// every output byte under the same visibility contract.
    ///
    /// # Safety
    /// `out` must be a 64-byte window of the same allocation from which this
    /// plan was prepared.  In particular, its alignment must match `nt`.
    #[inline(always)]
    pub unsafe fn write_zero_window(self, out: &mut [u8; 64]) {
        #[cfg(all(
            target_arch = "x86_64",
            target_feature = "avx512f",
            target_feature = "avx512bw"
        ))]
        // SAFETY: forwarded from this method's contract; zero is the complete
        // transformed value of a padding-only window.
        unsafe {
            kernels::x86_64::store_out64(
                out,
                core::arch::x86_64::_mm512_setzero_si512(),
                self.nt,
            );
        }
        #[cfg(not(all(
...
/// Opt-in isolation switch for the ranked identity-C Fold8 Horner collapse.
/// The canonical sidecar stays live independently; the official combined
/// sample regressed, so the wider arithmetic rewrite is no longer default.
const ENV_ENABLE_ZC_C_FOLD8_MEDIUM_HORNER: &str = "FLOCK_ENABLE_ZC_C_FOLD8_MEDIUM_HORNER";

#[inline]
fn ranked_c_fold8_medium_horner_enabled() -> bool {
    static ON: std::sync::LazyLock<bool> = std::sync::LazyLock::new(|| {
        std::env::var_os(ENV_ENABLE_ZC_C_FOLD8_MEDIUM_HORNER).as_deref()
            == Some(std::ffi::OsStr::new("1"))
    });
    *ON
}

/// Collapse the ranked identity-C Fold8 tensor without materialising its
/// Fold4 and quad intermediates.
///
/// The four retained medium coordinates are the protocol constants
/// `beta_i = gamma^(2^i) / (1 + gamma^(2^i))`, hence their sixteen eq
/// weights are `D^-1 * gamma^q`. For each of the four small-coordinate
/// banks, Horner evaluation therefore replaces sixteen general field
/// products per packed value with fifteen `mul_by_x` shifts. Only the four
/// final small-eq scalings remain general products.
fn collapse_ranked_c_fold8_medium_horner(fold8: &[F128], inner_tail: &[F128]) -> Vec<F128> {
    let n_packed = 1usize << crate::pcs::LOG_PACKING;
    assert_eq!(fold8.len(), 64 * n_packed);
    assert!(inner_tail.len() >= 6);
    debug_assert_eq!(&inner_tail[2..6], medium_challenges_ghash().as_slice());

    let small_eq = build_eq(&inner_tail[..2]);
    let mut out = vec![F128::ZERO; n_packed];
    let mut acc = [F128::ZERO; 1usize << crate::pcs::LOG_PACKING];
    let medium_scale = d_inv();
    for small_bank in 0..4 {
        let last = (small_bank + 4 * 15) * n_packed;
        acc.copy_from_slice(&fold8[last..last + n_packed]);
        for q in (0..15).rev() {
            let base = (small_bank + 4 * q) * n_packed;
            for (value, &coefficient) in acc.iter_mut().zip(&fold8[base..base + n_packed]) {
                *value = mul_by_x(*value) + coefficient;
            }
        }
        crate::field::f128_slice::add_scaled(&mut out, &acc, small_eq[small_bank] * medium_scale);
    }
    out
}
...
pub fn round1_c_fold4_from_block_major_z(
    ...
    static MEDIUM_CHALLENGES: std::sync::LazyLock<[F128; 4]> =
        std::sync::LazyLock::new(medium_challenges_ghash);
    let fold8_medium_horner = crate::pcs::ranked_direct_fold8_enabled()
        && ranked_c_fold8_medium_horner_enabled()
        && inner_tail.get(2..6) == Some(MEDIUM_CHALLENGES.as_slice());
    let (fold4, fold8) = if crate::pcs::ranked_direct_fold8_enabled() {
        ...
        let fold4 = if fold8_medium_horner {
            Vec::new()
        } else {
            // Rollback path: collapse retained coordinates 4 and 5 to
            // recover Fold4 exactly.
    ...
    let (s_hat_v_c, quad) = if fold8_medium_horner {
        (
            collapse_ranked_c_fold8_medium_horner(&fold8, inner_tail),
            Vec::new(),
        )
    } else {
        // Rollback and Fold4 paths: recover the incumbent four-bank tensor
        // (coordinates 0 and 1 stay bank selectors), then collapse it.
...
mod tests {
    ...
    #[test]
    fn ranked_c_fold8_medium_horner_matches_generic_collapse() {
        let mut rng = Rng::new(0xC0FF_EE80_F01D_8008);
        let fold8 = rng.f128_vec(64 * (1usize << crate::pcs::LOG_PACKING));
        let small = small_challenges_ghash();
        let medium = medium_challenges_ghash();
        let low_point = [
            small[1], small[2], medium[0], medium[1], medium[2], medium[3],
        ];
        assert_eq!(
            collapse_ranked_c_fold8_medium_horner(&fold8, &low_point),
            crate::pcs::ring_switch::collapse_s_hat_v_fold8(&fold8, &low_point),
        );
    }
    ...
    fn identity_c_fold_matches_row_major_fold4_drain() {
        ...
        for (m, useful_bits) in [(22usize, 15_409usize), (24, 15_409), (22, 1usize << 14)] {
            ...
            if crate::pcs::ranked_direct_fold8_enabled() && ranked_c_fold8_medium_horner_enabled() {
                assert!(quad_new.is_empty(), "Horner path still materialised quad");
                assert!(fold4_new.is_empty(), "Horner path still materialised Fold4");
            } else {
                assert_eq!(
                    quad_new, quad_ref,
