...
fn process_one_x_hi_ab_only(
    ...
    for x_outer_lo in 0..big_lo_size {
        ...
        // Direct zero-copy reference to this window's 16 contiguous 64-byte chunks.
        // Each window is 1024 bytes (16 * 64) with 64-byte alignment guaranteed by scratch.
        // Any padding chunks past n_b_med are guaranteed zero-filled in precompute_round1_ab_inner_packed_padded.
        let ab_window: &[[u8; 64]; 16] = unsafe {
        ...
        if let Some((_, mats, bank_bits)) = eq_fold {
            ...
            if plane_first_write && w_idx == 0 {
                kernels::write_convert_ab_nomul_gfni(ab_window, n_b_med, mats_w, bank);
            } else {
                kernels::accumulate_convert_ab_nomul_gfni(
                    ab_window,
                    n_b_med,
