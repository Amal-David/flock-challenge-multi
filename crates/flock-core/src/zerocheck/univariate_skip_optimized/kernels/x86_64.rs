...
unsafe fn accumulate_convert_ab_nomul_x86_gfni_impl<const FIRST_WRITE: bool>(
    ...
    unsafe {
        ...
        // Look-ahead T0s of the next packed window, issued after the current
        // window's stack loadus so SPR LFBs are not shared with that burst.
        // Not inside the plane loop: a prefetch has no architectural effect
        // and must not add a per-plane branch on the GFNI battery.
        for k in 0..pf_n {
