...
/// Two-window wavefront of [`shift_reduce_inner_ab_x86_avx512`].
///
/// Windows `b_med` and `b_med+1` share the `x^k` scale broadcast and are
/// otherwise independent. One K-loop keeps two ZMM accumulators in flight
/// so the GFNI mul pipe sees two independent products per step instead of
/// two sequential 8-step passes. Same applies, same muls, same XOR — bit
/// identical to two singles.
///
/// # Safety
/// As for [`shift_reduce_inner_ab_x86_avx512`], for both windows.
#[inline]
#[cfg(all(
    target_arch = "x86_64",
    target_feature = "gfni",
...
pub(crate) unsafe fn shift_reduce_inner_ab_x86_avx512_x2(
    ...
    // SAFETY: caller packed-input bounds cover 8 readable bytes at every
    // K-row of both windows; each `out*` is one writable ZMM.
    unsafe {
        ...
        for k in 0..8usize {
            ...
            let (s0, s1) = if k == 0 {
                ...
                (
                    _mm512_gf2p8mul_epi8(p0, xk),
                    _mm512_gf2p8mul_epi8(p1, xk),
                )
            };
...
mod usk_x2_tests {
    use super::{
        shift_reduce_inner_ab_x86_avx512, shift_reduce_inner_ab_x86_avx512_x2, InvNttTableByteSingleGf8,
    };
    ...
    fn shift_reduce_inner_ab_x86_avx512_x2_matches_two_singles() {
        ...
        let mut state = 0xC0FF_EE12_3456_7890u64;
        let mut next = || {
            state = state
        ...
        // 2 outer windows × 16 b_med × 8 K-rows × 8 bytes.
        ...
        for x in 0..2 {
            for b_med in [0usize, 2, 4, 6, 8, 10, 12] {
                let mut want0 = [0u8; 64];
                let mut want1 = [0u8; 64];
                let mut got0 = [0u8; 64];
                let mut got1 = [0u8; 64];
                // SAFETY: avx512f+avx512bw+gfni cfg-guaranteed; buffers cover
                // both windows at `x * OUTER + b_med * 64`.
                unsafe {
                    shift_reduce_inner_ab_x86_avx512(
                        &a, &b, &inv_table, x * OUTER, b_med, &mut want0,
                    );
                    shift_reduce_inner_ab_x86_avx512(
                        &a, &b, &inv_table, x * OUTER, b_med + 1, &mut want1,
                    );
                    shift_reduce_inner_ab_x86_avx512_x2(
                        &a, &b, &inv_table, x * OUTER, b_med, &mut got0, &mut got1,
                    );
                }
                assert_eq!(got0, want0, "out0 x={x} b_med={b_med}");
                assert_eq!(got1, want1, "out1 x={x} b_med={b_med}");
            }
        }
    }
}
