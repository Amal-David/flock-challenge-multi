...
///
/// Equivalence: `kernel_x2_all_ones` is two `kernel::<0>` bodies sharing
/// one K-loop and one `partials.rows[0]` load; `kernel_x2_tail` is
/// `kernel::<30>` (one `apply_full` + one GFNI mul) plus `kernel::<31>`
/// (store-zero). Same sniff-then-compute contract as the singles.
///
/// # Safety
