...
pub(super) fn accumulate_convert_ab_nomul_gfni(
    ...
    accumulate_convert_ab_nomul_gfni_pf(
        chunk_ab_bytes,
        n_b_med,
        mats,
        bank_planes,
...
/// [`accumulate_convert_ab_nomul_gfni`] plus look-ahead T0s of `pf_n`
/// packed 64-byte rows at `pf_next`, issued after the current window's
/// stack loadus and before the GFNI plane battery. `pf_n == 0` is a no-op.
#[cfg(all(
    target_arch = "x86_64",
    target_feature = "avx512f",
    target_feature = "vpclmulqdq",
...
#[inline]
pub(super) fn accumulate_convert_ab_nomul_gfni_pf(
    chunk_ab_bytes: &[[u8; 64]; 16],
    n_b_med: usize,
    mats: &[u64; 256],
    bank_planes: &mut [u8; 16 * 64],
    ...
    // cover every 64-byte load/store. Prefetch is a hint.
    unsafe {
        x86_64::accumulate_convert_ab_nomul_x86_gfni(
            chunk_ab_bytes,
            n_b_med,
            mats,
            bank_planes,
...
pub(super) fn write_convert_ab_nomul_gfni(
    ...
    write_convert_ab_nomul_gfni_pf(
        chunk_ab_bytes,
        n_b_med,
        mats,
        bank_planes,
...
/// First-visit twin of [`accumulate_convert_ab_nomul_gfni_pf`].
#[cfg(all(
    target_arch = "x86_64",
    target_feature = "avx512f",
    target_feature = "vpclmulqdq",
...
#[inline]
pub(super) fn write_convert_ab_nomul_gfni_pf(
    chunk_ab_bytes: &[[u8; 64]; 16],
    n_b_med: usize,
    mats: &[u64; 256],
    bank_planes: &mut [u8; 16 * 64],
