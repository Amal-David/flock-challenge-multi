...
>(
    ...
    unsafe {
        ...
        while lane < lanes {
            ...
            // two at a time at eight points spaced through the body instead
            ...
            // `zc_tail_pf_spread`) already spread their hint delivery and
            ...
            let twiddle = tw[1];
            for i in 0..4 {
                butterfly!(i, i + 4, twiddle);
            }
            pf_pair!(3);
            let twiddle = tw[2];
            for i in 0..4 {
                butterfly!(8 + i, 8 + i + 4, twiddle);
            }
            pf_pair!(4);
            for s in 0..2 {
                let twiddle = tw[3 + s];
