...
/// x86-only dispatchers for the ported seed row kernels (used by the x86
/// from-message tiled top pass; see `seed_and_top_tiled_x86_from_message`).
///
/// # Safety
/// Same source/destination validity, non-aliasing, and disjoint-write
/// contract as the other row kernels.
#[cfg(all(
    target_arch = "x86_64",
    target_feature = "avx512f",
    target_feature = "vpclmulqdq"
...
pub(super) unsafe fn butterfly_fused_2layer_row_from_sparse(
    ...
    // SAFETY: forwarded caller contract.
    unsafe {
        x86_64::butterfly_fused_2layer_row_from_sparse(src, dst, quarter, num_ntts, r, right_twiddle);
    }
}

/// Lanes-limited dispatcher for the static zero-lane skip (x86-only; see the
/// x86 kernel's docs).
///
/// # Safety
/// Same contract as [`butterfly_fused_4layer_row`], with `lanes <= num_ntts`.
#[cfg(all(
    target_arch = "x86_64",
    target_feature = "avx512f",
    target_feature = "vpclmulqdq"
...
pub(super) unsafe fn butterfly_fused_4layer_row_lanes(
    ...
    // SAFETY: forwarded caller contract.
    unsafe {
        x86_64::butterfly_fused_4layer_row_lanes(ptr, sixteenth, num_ntts, r, twiddles, lanes);
    }
}
