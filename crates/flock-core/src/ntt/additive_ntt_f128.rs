...
fn st_fmp_run(
    ...
    rayon::broadcast(|ctx| {
        ...
        let mut i = 0usize;
        let mut pending_r = n_tasks;
        loop {
            let r = if pending_r < n_tasks {
                let r = pending_r;
                pending_r = n_tasks;
                r
            } else {
                let r = next.fetch_add(2, Ordering::Relaxed);
                if r >= n_tasks {
                    break;
                }
                pending_r = r + 1;
...
impl AdditiveNttF128 {
    ...
    fn forward_transform_interleaved_parallel_from_layer_impl(
        ...
        let deep_sub = |sub_idx: usize,
                        ...
                        hint: u8|
         -> bool {
            if let (true, Some(cb)) = (fuse_blocks, block_cb) {
