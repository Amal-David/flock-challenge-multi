         ...
         -> bool {
            if fuse_l1_blocks && block_cb.is_some() {
                ...
                for b in 0..16usize {
                    ...
                    // All 1,024 leaves are final. This exact aligned range
                    // lets the generic recursive-commit callback hash the
                    // hot 128 KiB and fold ten local Merkle levels. The
                    // callback has no observable completion order.
                    let lo = sub_idx * sub_size_positions + b * block_size8;
                    cb(lo..lo + block_size8, &*blk);
                }
                return true;
            }
