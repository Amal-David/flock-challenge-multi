...
impl AdditiveNttF128 {
    ...
    fn seed_rate_quarter_top_fused8_pass(
        ...
        const STAGE_ROWS: usize = TOP_BLOCKS * ROWS_PER_BLOCK;
        ...
        let task = |buf: &mut Vec<F128>, r: usize| {
            ...
            // SAFETY: `r < sub_stride` and `k < 64` partition every source
            // row-group exactly once. The four seed blocks write the sixteen
            // disjoint 64-row staging regions; this task owns all of them.
            unsafe {
                ...
                for k in 0..ROWS_PER_BLOCK {
                    ...
                    for (rate_block, tw) in seed_tw.iter().enumerate().skip(1) {
                        let first_top_block = 4 * rate_block;
                        kernels::butterfly_fused_2layer_row_from_geo(
                            src,
                            block_size,
                            src_r,
                            bufp.add(
                                (first_top_block * ROWS_PER_BLOCK + kp) * row_len,
                            ),
                            ROWS_PER_BLOCK,
                            0,
                            row_len,
                ...
                // Each layer-4 block is now a complete 64-row cache-resident
                // transform. Preserve the incumbent per-row layer order.
                for block in 0..TOP_BLOCKS {
                    let region = bufp.add(block * ROWS_PER_BLOCK * row_len);
                    for j in 0..4 {
                        kernels::butterfly_fused_4layer_row(
                            region.add(j * g4_base * row_len),
                            g4_stride,
                    ...
                    for m in 0..16 {
                        ...
                        let t_inner_a = self.twiddle(TOP_LAYER + 5, 2 * outer_block);
                        let t_inner_b = self.twiddle(TOP_LAYER + 5, 2 * outer_block + 1);
                        let p = region.add(perm(4 * m) * row_len);
                        let step = g2_stride * row_len;
                        let a = std::slice::from_raw_parts_mut(p, row_len);
                        let b = std::slice::from_raw_parts_mut(p.add(step), row_len);
                        let c = std::slice::from_raw_parts_mut(p.add(2 * step), row_len);
                        let d = std::slice::from_raw_parts_mut(p.add(3 * step), row_len);
                ...
                // Publish each final row once. The per-job guard drains this
                // worker's NT writes before the Rayon producer region joins the
                // deep pass, which may read the rows on different workers.
                let base = base_addr as *mut F128;
                if publish_nt {
                    for block in 0..TOP_BLOCKS {
                        for k in 0..ROWS_PER_BLOCK {
                            Self::publish_row_nt(
                                bufp.add(
                                    (block * ROWS_PER_BLOCK + perm(k)) * row_len,
                                ),
                                base.add(
                                    seed_top_codeword_row(
                                        block,
                                        r,
                                        k,
                                        block_size,
                ...
                for block in 0..TOP_BLOCKS {
                    for k in 0..ROWS_PER_BLOCK {
                        core::ptr::copy_nonoverlapping(
                            bufp.add((block * ROWS_PER_BLOCK + perm(k)) * row_len),
                            base.add(block * block_elems + (r + k * sub_stride) * row_len),
                            row_len,
                        );
                    }
                }
