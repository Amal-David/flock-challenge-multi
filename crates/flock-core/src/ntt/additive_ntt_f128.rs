...
///
/// **Three is the remote-only one-factor pipeline-slack probe.** Today `for_each_init`
...
/// M+P sibling allocates, and it allocates `nbuf` blocks. This candidate raises
/// the default from two to three blocks, buying one block of producer/consumer
/// slack at a 1.5 MiB footprint per physical core. That deliberately probes the
/// risk against Sapphire Rapids' 2 MiB private L2; the environment override can
/// still select two or four for same-binary diagnostics.
