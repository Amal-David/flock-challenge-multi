...
impl AdditiveNttF128 {
    ...
    /// `FLOCK_NO_NTT_SCATTER_ZMM=1` restores XMM-only NT publish (the
    /// historical loop that never peels to a 64-byte cursor).
    #[cfg(all(target_arch = "x86_64", target_feature = "avx512f"))]
    fn scatter_zmm_enabled() -> bool {
        static ON: std::sync::LazyLock<bool> =
            std::sync::LazyLock::new(|| std::env::var_os("FLOCK_NO_NTT_SCATTER_ZMM").is_none());
        *ON
    }
