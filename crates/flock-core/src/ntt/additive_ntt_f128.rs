...
/// `FLOCK_NO_NTT_RATE4_SEED_TOP_FUSION=1` restores the separate rate-1/4
/// seed plus deep-pass schedule for the exact ranked L1 recursive commit.
/// This switch is deliberately narrower than `FLOCK_NO_NTT_RATE_SEED`: the
/// latter disables every rate <= 1/4 seed fast path.
#[cfg(all(
    target_arch = "x86_64",
...
fn ntt_rate4_seed_top_fusion_disabled() -> bool {
    static OFF: OnceLock<bool> = OnceLock::new();
    *OFF.get_or_init(|| std::env::var_os("FLOCK_NO_NTT_RATE4_SEED_TOP_FUSION").is_some())
}
...
///
/// **Why the zero-init is dead.** All callers fill the whole block before
/// reading any of it:
///
...
/// - `seed_rate_quarter_top_fused8_pass` repeats the same construction for four
///   rate blocks: each `k` writes four rows in each of the four disjoint
///   256-row regions, covering exactly rows `0..1024` and all eight lanes before
///   the first staged butterfly reads them.
///
...
impl AdditiveNttF128 {
    ...
    pub(crate) fn rs_encode_interleaved_on_range_done(
        ...
        // Exact ranked L1 recursive commit: rate 1/4, 2^18 positions, eight
        // interleaved lanes (8 MiB message -> 32 MiB codeword). Fold the two
        // seeded layers and layers 4..9 into one cache-blocked producer pass,
        // then let the incumbent deep path finish at layer 10. Keeping its
        // native n_top preserves the existing Merkle callback ranges.
        #[cfg(all(
            target_arch = "x86_64",
            target_feature = "pclmulqdq",
            target_feature = "avx512f",
        ...
        if log_inv_rate == 2
            ...
            && !rate_seed_disabled()
            && !ntt_top_fusion_disabled()
            && !ntt_rate4_seed_top_fusion_disabled()
        {
            self.seed_rate_quarter_top_fused8_pass(msg, codeword, num_ntts, log_d);
            self.forward_transform_interleaved_parallel_from_layer_impl(
                codeword,
                num_ntts,
                10,
    ...
    /// Exact ranked-L1 sibling of [`Self::seed_top_fused8_pass`]. The rate-1/4
    /// seed computes layers 2 and 3 from the 8 MiB message; this task keeps
    /// those rows in a 128 KiB private staging block, applies layers 4..9, and
    /// publishes the 32 MiB codeword once at the post-layer-9 state.
    ///
    /// Geometry: after the seed there are sixteen layer-4 blocks. One task owns
    /// a row residue `r` and gathers 64 source row groups. Each source group
    /// produces four rows in each of the four rate blocks, so the task fills
    /// `16 * 64` staging rows exactly once. The six following layers are then
    /// independent inside each 64-row block. Distinct `r` values scatter to
    /// disjoint final codeword rows.
    #[cfg(all(
        target_arch = "x86_64",
        target_feature = "pclmulqdq",
        target_feature = "avx512f",
    ...
    fn seed_rate_quarter_top_fused8_pass(
        &self,
        msg: &[F128],
        data: &mut [F128],
        num_ntts: usize,
        ...
        const TOP_BLOCKS: usize = 16;
        const ROWS_PER_BLOCK: usize = 64;
        const R_CHUNK: usize = 16;
        const STAGE_BLOCKS: usize = 4;
        const STAGE_ROWS: usize = STAGE_BLOCKS * ROWS_PER_BLOCK * R_CHUNK;
        ...
        let msg_positions = msg.len() / num_ntts;
        debug_assert_eq!(msg_positions, 4 * block_size);
        debug_assert!(block_size.is_multiple_of(ROWS_PER_BLOCK));
        let sub_stride = block_size / ROWS_PER_BLOCK;
        debug_assert!(sub_stride > 0);
        let row_len = num_ntts;

        // The four copy-only rate blocks share the same message rows, but each
        // has its own layer-2/layer-3 twiddle triple.
        ...
        // Layers 4..7 use the fused-four kernel. Layers 8..9 are looked up in
        // the task immediately before the final fused-two/publish phase.
        ...
        struct Rate4SeedTopNtFence(bool);
        impl Drop for Rate4SeedTopNtFence {
            fn drop(&mut self) {
                if self.0 {
                    // This Rayon job's NT writes all ran on this worker. Drain
                    // once before the producer region can join the deep pass.
                    unsafe { core::arch::x86_64::_mm_sfence() };
                }
            }
        }

        let r_chunks = sub_stride / R_CHUNK;
        debug_assert_eq!(sub_stride % R_CHUNK, 0);
        let task = |buf: &mut Vec<F128>, task_idx: usize| {
            debug_assert_eq!(buf.len(), STAGE_ROWS * row_len);
            let rate_block = task_idx / r_chunks;
            let r0 = (task_idx % r_chunks) * R_CHUNK;
            debug_assert!(rate_block < RATE_BLOCKS);
            let first_top_block = STAGE_BLOCKS * rate_block;
            let bufp = buf.as_mut_ptr();
            // Stage one rate block across sixteen adjacent residues. Layout is
            // [layer-4 child][permuted k][residue], so both the source and
            // final destination of each 16-row publication burst are
            // contiguous while every residue still owns an independent
            // 64-row layers-4..9 transform.
            unsafe {
                let src = src_addr as *const F128;
                let seed_stride = ROWS_PER_BLOCK * R_CHUNK;
                for rr in 0..R_CHUNK {
                    for k in 0..ROWS_PER_BLOCK {
                        let src_r = r0 + rr + k * sub_stride;
                        let dst = bufp.add((perm(k) * R_CHUNK + rr) * row_len);
                        if rate_block == 0 {
                ...
                // Each residue is an independent 64-row transform. Multiplying
                // the incumbent stage strides by R_CHUNK walks its rows in the
                // interleaved [k][residue] layout without changing arithmetic.
                for child in 0..STAGE_BLOCKS {
                    let block = first_top_block + child;
                    for rr in 0..R_CHUNK {
                        let region = bufp.add(
                            (child * ROWS_PER_BLOCK * R_CHUNK + rr) * row_len,
                        );
                        for j in 0..4 {
                            kernels::butterfly_fused_4layer_row(
                                region.add(j * g4_base * R_CHUNK * row_len),
                                g4_stride * R_CHUNK,
                                row_len,
                                row_len,
                                0,
                        ...
                        for m in 0..16 {
                            let outer_block = block * 16 + m;
                            let t_outer = self.twiddle(TOP_LAYER + 4, outer_block);
                            let t_inner_a =
                                self.twiddle(TOP_LAYER + 5, 2 * outer_block);
                            let t_inner_b =
                                self.twiddle(TOP_LAYER + 5, 2 * outer_block + 1);
                            let p =
                                region.add(perm(4 * m) * R_CHUNK * row_len);
                            let step = g2_stride * R_CHUNK * row_len;
                            let a = std::slice::from_raw_parts_mut(p, row_len);
                            let b =
                                std::slice::from_raw_parts_mut(p.add(step), row_len);
                ...
                // Publish 16 adjacent residues as one burst for every logical
                // k row. The previous residue-owned task wrote one 128-byte row
                // every 32 KiB; this ownership keeps both stage reads and
                // codeword writes contiguous without changing row indices.
                let base = base_addr as *mut F128;
                for child in 0..STAGE_BLOCKS {
                    let block = first_top_block + child;
                    for k in 0..ROWS_PER_BLOCK {
                        let srcp = bufp.add(
                            (child * ROWS_PER_BLOCK * R_CHUNK
                                + perm(k) * R_CHUNK)
                                * row_len,
                        );
                        let dstp = base.add(
                            (block * block_size + r0 + k * sub_stride) * row_len,
                        );
                        if publish_nt {
                            for rr in 0..R_CHUNK {
                                Self::publish_row_nt(
        ...
        (0..RATE_BLOCKS * r_chunks)
            .into_par_iter()
            .for_each_init(
                || {
                    (
                        staging_block(STAGE_ROWS, row_len),
                        Rate4SeedTopNtFence(publish_nt),
                    )
                },
                |state, task_idx| task(&mut state.0, task_idx),
            );
    }
