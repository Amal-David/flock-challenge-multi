...
impl AdditiveNttF128 {
    ...
    /// x86 tiled top pass over a PREINITIALIZED codeword: layers 1..9 of the
    /// ranked L0 transform in ONE memory sweep. The 256-row super-tile
    /// {r + j*(d/512), j = 0..256} inside each layer-1 block is closed under
    /// layers 1..8, so each tile applies the fused-four kernel for layers
    /// 1..5 (16 calls) and again for layers 5..9 (16 calls, one per layer-5
    /// block slice) while its 256 KiB stay cache-resident. The generic
    /// schedule runs the same layers as two full-1-GiB read+write sweeps.
    /// Same kernels, same twiddles, same per-element operation order --
    /// bit-identical output.
    ///
    /// Caller contract: `log_d == 20` (ranked L0), codeword holds the exact
    /// replicate-fill state, and a follow-up transform from layer 9.
    pub(crate) fn top_tiled_x86_preinitialized(&self, data: &mut [F128], num_ntts: usize) {
        #[cfg(all(
            target_arch = "x86_64",
        ...
        ))]
        {
            ...
            let block1_size = 1usize << (log_d - 1);
            let unit = 1usize << (log_d - 9);
            let sixteenth1 = block1_size >> 4;
            let block5_size = 1usize << (log_d - 5);
            let sixteenth5 = block5_size >> 4;
            debug_assert_eq!(sixteenth5, unit);

            // Twiddles: layers 1..5 per layer-1 block (2), layers 5..9 per
            // layer-5 block (32 = 2 x 16).
            let mut tw_top = [[F128::ZERO; 15]; 2];
            for (b1, tw) in tw_top.iter_mut().enumerate() {
                tw[0] = self.twiddle(1, b1);
                for s in 0..2 {
                    tw[1 + s] = self.twiddle(2, 2 * b1 + s);
                }
                for s in 0..4 {
                    tw[3 + s] = self.twiddle(3, 4 * b1 + s);
                }
                for s in 0..8 {
                    tw[7 + s] = self.twiddle(4, 8 * b1 + s);
                }
            }
            let mut tw_mid = vec![[F128::ZERO; 15]; 32];
            for (gb5, tw) in tw_mid.iter_mut().enumerate() {
                tw[0] = self.twiddle(5, gb5);
                for s in 0..2 {
                    tw[1 + s] = self.twiddle(6, 2 * gb5 + s);
                }
                for s in 0..4 {
                    tw[3 + s] = self.twiddle(7, 4 * gb5 + s);
                }
                for s in 0..8 {
                    tw[7 + s] = self.twiddle(8, 8 * gb5 + s);
                }
            }

            let base_addr = data.as_mut_ptr() as usize;
            // Static zero-lane skip: tile strides are even, so every row in a
            // task's tile shares the task's `r` parity; odd tiles process
            // only the nonzero lane prefix (see `ranked_zero_odd_tail_lanes`).
            let odd_tail = ranked_zero_odd_tail_lanes(log_d, num_ntts);
            (0..2 * unit).into_par_iter().for_each(|task| {
                let b1 = task / unit;
                let r = task % unit;
                let lanes = if odd_tail > 0 && r & 1 == 1 {
                    num_ntts - odd_tail
                } else {
                    num_ntts
                };
                // SAFETY: each task owns the disjoint 256-row tile
                // {r + j*unit} inside layer-1 block b1; both stages write
                // only those rows, sequentially within the task.
                unsafe {
                    let bp = (base_addr as *mut F128).add(b1 * block1_size * num_ntts);
                    for j0 in 0..16 {
                        kernels::butterfly_fused_4layer_row_lanes(
                            bp,
                            sixteenth1,
                            num_ntts,
                            r + j0 * unit,
        ...
        {
            let _ = (data, num_ntts);
            unreachable!("top_tiled_x86_preinitialized is x86-only");
        }
    }

    /// x86 fused seed + six-layer tiled top pass: layers 1..9 of the ranked
    /// rate-1/2 L0 transform in ONE memory sweep. Per task, 64 seed row
    /// groups synthesize the 512-row super-tile {k*(d/8) + r + j*(d/512)}
    /// straight from the message (one row per layer-3 block per group,
    /// regular stores), then layers 3..9 run on each block's 64-row tile
    /// while it is cache-hot. The message is read once and the codeword
    /// written once for all of layers 1..9, versus replicate-fill (a full
    /// extra write pass) plus two read+write top sweeps in the generic
    /// schedule. Same kernels, same twiddles, same per-element operation
    /// order as that schedule -- bit-identical output.
    ///
    /// Caller contract: rate 1/2 (`data.len() == 2 * msg.len()`), the ranked
    /// L0 shape (`log_d == 20`), and a follow-up transform from layer 9.
    pub(crate) fn seed_and_top_tiled_x86_from_message(
        &self,
        msg: &[F128],
        data: &mut [F128],
        num_ntts: usize,
        ...
        ))]
        {
            ...
            let n_total = data.len();
            let log_d = log2_pow2(n_total / num_ntts);
            debug_assert_eq!(n_total, 2 * msg.len());
            debug_assert!(log_d >= 12);
            let block_size = 1usize << (log_d - 3);
            let sixteenth = block_size >> 4;
            let sixty_fourth = block_size >> 6;
            let seed_quarter = (msg.len() / num_ntts) >> 2;
            debug_assert_eq!(seed_quarter, block_size);
            let half_len = msg.len();
            ...
            let mut tw4 = [[F128::ZERO; 15]; 8];
            let mut tw2 = [[[F128::ZERO; 3]; 16]; 8];
            for block in 0..8 {
                tw4[block][0] = self.twiddle(3, block);
                for s in 0..2 {
                    tw4[block][1 + s] = self.twiddle(4, 2 * block + s);
                }
                for s in 0..4 {
                    tw4[block][3 + s] = self.twiddle(5, 4 * block + s);
                }
                for s in 0..8 {
                    tw4[block][7 + s] = self.twiddle(6, 8 * block + s);
                }
                for (s, t) in tw2[block].iter_mut().enumerate() {
                    let gb = 16 * block + s;
                    t[0] = self.twiddle(7, gb);
                    t[1] = self.twiddle(8, 2 * gb);
                    t[2] = self.twiddle(8, 2 * gb + 1);
                }
            }

            let src_addr = msg.as_ptr() as usize;
            let base_addr = data.as_mut_ptr() as usize;
            // Static zero-lane skip for the tiled layers: the seed writes
            // below stay dense (they CREATE the zero tails on the stale
            // buffer); the fused stages then skip the zero lanes on
            // odd-parity tiles (all tile strides are even).
            let odd_tail = ranked_zero_odd_tail_lanes(log_d, num_ntts);
            (0..sixty_fourth).into_par_iter().for_each(|r| {
                // SAFETY: each task owns the disjoint seed groups
                // {r + j*sixty_fourth} and therefore the disjoint 512-row
                // super-tile they write; the tiled kernels read and write
                // only those rows, after the seed writes complete
                // (sequential in-task, regular stores).
                unsafe {
                    let sp = src_addr as *const F128;
                    let dp = base_addr as *mut F128;
                    for j in 0..64 {
                        let rp = r + j * sixty_fourth;
                    ...
                    for (block, (t4, t2)) in tw4.iter().zip(tw2.iter()).enumerate() {
                        let bp = dp.add(block * block_size * num_ntts);
                        for j0 in 0..4 {
                            kernels::butterfly_fused_4layer_row_lanes(
                                bp,
                                sixteenth,
                        ...
                        for (s, t) in t2.iter().enumerate() {
                            let row0 = (s * sixteenth + r) * num_ntts;
                            let stride = sixty_fourth * num_ntts;
                            let a = std::slice::from_raw_parts_mut(bp.add(row0), lanes);
                            let b = std::slice::from_raw_parts_mut(
                                bp.add(row0 + stride),
                                lanes,
                            );
                            let c = std::slice::from_raw_parts_mut(
                                bp.add(row0 + 2 * stride),
                                lanes,
                            );
                            let d = std::slice::from_raw_parts_mut(
                                bp.add(row0 + 3 * stride),
                                lanes,
                            );
                            kernels::butterfly_fused_2layer(a, b, c, d, t[0], t[1], t[2]);
                        }
                    }
                }
            });
        ...
        {
            let _ = (msg, data, num_ntts);
            unreachable!("seed_and_top_tiled_x86_from_message is x86-only");
        }
    }
    ...
    fn forward_transform_interleaved_deep_subtree(
        ...
        // x86 AVX-512: greedy fused-four schedule — a row group stays in
        // registers across four butterfly layers, so the ranked deep layers
        // 9..19 run as two fused-four sweeps, one fused-two sweep, and one
        // scalar tail instead of eleven single-layer subgroup sweeps. Same
        // butterflies, same twiddles, same per-element operation order.
        let fused4_ok = cfg!(all(
            target_arch = "x86_64",
            target_feature = "avx512f",
            target_feature = "vpclmulqdq"
        ));
        // Static zero-lane skip (see `ranked_zero_odd_tail_lanes`): odd
        // positions' trailing lanes are zero through every even-distance
        // layer, so those butterflies can skip their loads and stores. Block
        // bases are even and the row strides below are even wherever the
        // skip engages, so a row's position parity equals its local index
        // parity.
        let odd_tail = ranked_zero_odd_tail_lanes(log_d, num_ntts);
        if fused4_ok {
            ...
            while layer < log_d {
                ...
                if layer + 3 < log_d && block_size >= 16 {
                    let sixteenth = block_size >> 4;
                    for block_in_sub in 0..num_blocks_in_sub {
                        ...
                        let bp = sub_data[block_start..block_start + block_bytes].as_mut_ptr();
                        let tail_ok = odd_tail > 0 && sixteenth.is_multiple_of(2);
                        for r in 0..sixteenth {
                            let lanes = if tail_ok && r & 1 == 1 {
                                num_ntts - odd_tail
                            } else {
                                num_ntts
                            };
                            // SAFETY: rows (i*sixteenth + r) for i in 0..16 lie
                            // inside this block and are disjoint across r.
                            unsafe {
                                kernels::butterfly_fused_4layer_row_lanes(
                                    bp, sixteenth, num_ntts, r, &tw, lanes,
                                );
                            }
                        }
                    }
                    ...
                    for block_in_sub in 0..num_blocks_in_sub {
                        ...
                        let tail_ok = odd_tail > 0 && quarter.is_multiple_of(2);
                        for r in 0..quarter {
                            let lanes = if tail_ok && r & 1 == 1 {
                                num_ntts - odd_tail
                            } else {
                            ...
                            let row = r * num_ntts;
                            let (a, rest) = block[row..].split_at_mut(num_ntts);
                            let (b, rest) = rest[stride - num_ntts..].split_at_mut(num_ntts);
                            let (c, rest) = rest[stride - num_ntts..].split_at_mut(num_ntts);
                            let d = &mut rest[stride - num_ntts..stride];
...
mod tests {
    ...
    fn standard_dim20_final_pair_twiddles_have_zero_high_limbs() {
        ...
        // The narrow dispatch itself is mac-only (deep-pair fusion at
        // n_top = 10); the zero-high-limb table invariant above holds on
        // every target.
...
mod zero_lane_skip {
    ...
    /// The three-CLMUL zero-high-limb product must equal the generic product
    /// on every lane, for random low twiddles against random operands —
    /// covers the butterfly kernels' per-twiddle fast path.
    #[cfg(all(
        target_arch = "x86_64",
        target_feature = "avx512f",
        target_feature = "vpclmulqdq"
    ...
    #[test]
    fn ghash_mul_x4_low_matches_generic() {
        use crate::field::gf2_128::x86_64::{f128x4_set, ghash_mul_x4, ghash_mul_x4_low};
        let mut state = 0x1234_5678_9abc_def0u64;
        let mut next = || {
            state = state
        ...
        for _ in 0..64 {
            let tw: [F128; 4] = std::array::from_fn(|_| F128 { lo: next(), hi: 0 });
            let val: [F128; 4] = std::array::from_fn(|_| F128 {
                lo: next(),
                hi: next(),
            });
            // SAFETY: cfg gate supplies the target features.
            unsafe {
                let x = f128x4_set(tw[0], tw[1], tw[2], tw[3]);
                let y = f128x4_set(val[0], val[1], val[2], val[3]);
                let generic = ghash_mul_x4(x, y);
                let low = ghash_mul_x4_low(x, y);
                let mut g = [F128::ZERO; 4];
                let mut l = [F128::ZERO; 4];
                core::arch::x86_64::_mm512_storeu_si512(
                    g.as_mut_ptr().cast::<core::arch::x86_64::__m512i>(),
                    generic,
                );
                core::arch::x86_64::_mm512_storeu_si512(
                    l.as_mut_ptr().cast::<core::arch::x86_64::__m512i>(),
                    low,
                );
                assert_eq!(g, l);
            }
    ...
    /// Diagnostic: per-layer share of zero-high-limb twiddles at the ranked
    /// dim-20 standard NTT (run with --ignored --nocapture).
    #[test]
    #[ignore]
    fn ranked_twiddle_low_limb_stats() {
        let ntt = AdditiveNttF128::standard(20);
        for layer in 0..20 {
            let blocks = 1usize << layer;
            let mut low = 0usize;
            for b in 0..blocks {
                if ntt.twiddle(layer, b).hi == 0 {
                    low += 1;
                }
            }
            println!(
                "layer {layer:2}: {low:7}/{blocks:7} low ({:.1}%)",
                100.0 * low as f64 / blocks as f64
            );
        }
    }
