...
impl AdditiveNttF128 {
    ...
    pub(crate) unsafe fn seed_rate_quarter_rows_from_raw(
        ...
        if fuse_rate_blocks && num_ntts == 8 {
            // SAFETY: this is the same complete source/destination geometry
            // as the four-call loop below. The fused helper is specialized
            // to the ranked eight-way interleave and owns exactly `rows`.
            unsafe {
                kernels::butterfly_fused_2layer_rows_rate_quarter_8(
                    msg,
                    codeword,
                    msg_positions,
                    rows,
...
mod tests {
    ...
    fn producer_rate_quarter_rows_match_seed_and_finish_from_poison() {
        ...
        for &(log_msg_positions, num_ntts) in &[(4usize, 1usize), (6, 4), (8, 8)] {
            ...
            for fuse_rate_blocks in [false, true] {
                ...
                for pair in cuts.windows(2) {
                    ...
                    // complete geometry; these ranges are disjoint and cover
                    // the whole quarter exactly once.
                    unsafe {
