...
fn select_st_fmp_bufs(requested: Option<usize>) -> usize {
    // Three staging buffers, not two. The seed-and-top pass runs as a
    // producer/consumer split across each physical core's SMT pair, and this
    // is how many staging blocks the producer may have in flight. At two it
    // fills one while the consumer drains the other and then waits; a third
    // lets it keep going across the handoff.
    //
    // Half a mebibyte more per physical core against a 2 MiB private L2 is
    // affordable here because the prove is bound by memory rather than by the
    // vector units -- halving the pool costs 13.6% on this instance, not the
    // 50% a compute-bound prove would lose -- so a producer that stalls is
    // giving up issue slots the machine has spare, while the extra block it
    // holds is bytes the machine does not.
