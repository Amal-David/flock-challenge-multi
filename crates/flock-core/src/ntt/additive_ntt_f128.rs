...
impl AdditiveNttF128 {
    ...
    pub fn forward_transform_interleaved_parallel_from_layer(
        ...
        // Unit tests exercise the new deep schedule through its portable
        // oracle without changing the existing top-layer production schedule.
        ...
        data.par_chunks_mut(sub_bytes)
            ...
            .for_each(|(sub_idx, sub_data)| {
                // The cache-blocked tail used to sweep each subgroup once per
                // remaining layer. On AVX-512, reuse the same fused-four
                // kernel as the top layers so a row group stays in registers
                // across four butterflies. At the ranked geometry this turns
                // deep layers 9..19 from eleven subgroup sweeps into two
                // fused-four sweeps, one fused-two sweep, and one scalar tail.
                //
                // Keep the old schedule verbatim on other targets: the
                // portable fused-four kernel is scalar and loses to their
                // ordinary row-pair path.
                if deep_fused4_ok {
                    ...
                    while layer < log_d {
                        ...
                        if layer + 3 < log_d {
                            ...
                            // layers. Thus block_size <= 8 and quarter <= 2,
                            // keeping the existing helper sequential inside
                            // this outer Rayon subgroup task.
                            let quarter = block_size >> 2;
                            for block_in_sub in 0..num_blocks_in_sub {
                                let global_block = sub_idx * num_blocks_in_sub + block_in_sub;
