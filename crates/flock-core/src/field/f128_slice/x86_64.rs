...
pub(super) unsafe fn fold_pairs(src: &[F128], base: usize, dst: &mut [F128], r: F128) {
    ...
    unsafe {
        ...
        while t < lanes {
            ...
            let new = _mm512_xor_si512(even, ghash_mul_const_x4(_mm512_xor_si512(even, odd), r_bcast, r_x64));
...
pub(super) unsafe fn add_scaled(dst: &mut [F128], addend: &[F128], scale: F128) {
    ...
    unsafe {
        ...
        while i < lanes {
            ...
            let corrected = _mm512_xor_si512(current, ghash_mul_const_x4(extra, scale_x4, scale_x64));
...
pub(super) unsafe fn fold4_nested(src: &[F128], dst: &mut [F128], r0: F128, r1: F128) {
    ...
    // SAFETY: caller guarantees the target features and source bounds.
    unsafe {
        let (r0_bcast, r0_x64) = ghash_const_x4(r0);
        let (r1_bcast, r1_x64) = ghash_const_x4(r1);
        ...
        while t < lanes {
            ...
            let mid01 = _mm512_xor_si512(
                even01,
                ghash_mul_const_x4(_mm512_xor_si512(even01, odd01), r0_bcast, r0_x64),
            );
            ...
            let mid23 = _mm512_xor_si512(
                even23,
                ghash_mul_const_x4(_mm512_xor_si512(even23, odd23), r0_bcast, r0_x64),
            );
            ...
            let out = _mm512_xor_si512(low, ghash_mul_const_x4(_mm512_xor_si512(low, high), r1_bcast, r1_x64));
...
pub(super) unsafe fn fold_two_and_msg_in_place(
    ...
    use crate::field::gf2_128::x86_64::{ghash_const_x4, ghash_mul_const_x4, WideGhashX4};
    use core::arch::x86_64::*;
    ...
    unsafe {
        ...
        let fold4 = |ptr: *const F128, t: usize| -> __m512i {
            ...
            let even = _mm512_permutex2var_epi64(lo, idx_even, hi);
            let odd = _mm512_permutex2var_epi64(lo, idx_odd, hi);
            _mm512_xor_si512(even, ghash_mul_const_x4(_mm512_xor_si512(even, odd), r_bcast, r_x64))
        };
...
pub(super) unsafe fn bind_split_half(lo: &mut [F128], hi: &[F128], r: F128) {
    ...
    unsafe {
        ...
        while i < lanes {
            ...
            let new = _mm512_xor_si512(a, ghash_mul_const_x4(_mm512_xor_si512(a, b), r_bcast, r_x64));
...
pub(super) unsafe fn bind_both_and_msg_split(
    ...
    unsafe {
        ...
        while i < lanes {
            ...
            let lo = _mm512_xor_si512(c0, ghash_mul_const_x4(_mm512_xor_si512(c2, c0), r_bcast, r_x64));
            let hi = _mm512_xor_si512(c1, ghash_mul_const_x4(_mm512_xor_si512(c3, c1), r_bcast, r_x64));
            let zlo = _mm512_xor_si512(z0, ghash_mul_const_x4(_mm512_xor_si512(z2, z0), r_bcast, r_x64));
            let zhi = _mm512_xor_si512(z1, ghash_mul_const_x4(_mm512_xor_si512(z3, z1), r_bcast, r_x64));
