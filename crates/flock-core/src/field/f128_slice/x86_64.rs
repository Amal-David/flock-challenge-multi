...
///
/// The incumbent f-side T0 prefetch runs at the same four-output cadence and
/// depth as [`fold16_banked`]. Two T1 lines from `next_slab` are requested per
/// four b outputs, preserving the two promoted claim-loop visits; `None`
/// disables only that next-block stream.
///
/// # Safety
...
pub(super) unsafe fn fold16_banked_two_tables(
    ...
    debug_assert!(next_slab.is_none_or(|slab| slab.len() == src.len()));
    ...
    unsafe {
        ...
        let pf_ahead = fold16_pf_ahead();
        let pf_limit = src.len().saturating_sub(64);
        let (next_pf_base, next_pf_span) = next_slab.map_or(
        ...
        while t < f_dst.len() {
            ...
            if !next_pf_base.is_null() && next_pf_at < next_pf_span {
                _mm_prefetch::<_MM_HINT_T1>(next_pf_base.add(next_pf_at).cast::<i8>());
                next_pf_at += 64;
                if next_pf_at < next_pf_span {
                    _mm_prefetch::<_MM_HINT_T1>(next_pf_base.add(next_pf_at).cast::<i8>());
                    next_pf_at += 64;
                }
            }
