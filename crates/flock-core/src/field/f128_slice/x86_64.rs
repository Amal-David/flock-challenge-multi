...
pub(super) unsafe fn fold_pairs(src: &[F128], base: usize, dst: &mut [F128], r: F128) {
    ...
    unsafe {
        ...
        while t < lanes8 {
            ...
            let mul0 = ghash_mul_x4_split(diff0, r_bcast, r_x64);
            let mul1 = ghash_mul_x4_split(diff1, r_bcast, r_x64);

            let new0 = _mm512_xor_si512(even0, mul0);
            let new1 = _mm512_xor_si512(even1, mul1);
...
pub(super) unsafe fn add_scaled(dst: &mut [F128], addend: &[F128], scale: F128) {
    ...
    unsafe {
        ...
        while i < lanes8 {
            ...
            let mul0 = ghash_mul_x4_split(extra0, scale_x4, scale_x64);
            let mul1 = ghash_mul_x4_split(extra1, scale_x4, scale_x64);

            let corrected0 = _mm512_xor_si512(current0, mul0);
            let corrected1 = _mm512_xor_si512(current1, mul1);
...
pub(super) unsafe fn fold4_nested(src: &[F128], dst: &mut [F128], r0: F128, r1: F128) {
    ...
    unsafe {
        ...
        while t < lanes {
            ...
            let out = _mm512_xor_si512(low, ghash_mul_x4_split(_mm512_xor_si512(low, high), r1_bcast, r1_x64));
...
pub(super) unsafe fn fold_two_and_msg_in_place(
    ...
    use crate::field::gf2_128::x86_64::{ghash_mul_x4_split, ghash_shift64_x4, WideGhashX4};
    use core::arch::x86_64::*;
