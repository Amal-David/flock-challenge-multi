...
pub(super) unsafe fn fold_pairs(src: &[F128], base: usize, dst: &mut [F128], r: F128) {
    ...
    unsafe {
        ...
        while t < lanes8 {
            ...
            _mm512_storeu_si512(
                dst.as_mut_ptr().add(t) as *mut __m512i,
                _mm512_xor_si512(even0, mul0),
            );
            _mm512_storeu_si512(
                dst.as_mut_ptr().add(t + 4) as *mut __m512i,
                _mm512_xor_si512(even1, mul1),
            );
            t += 8;
        }
        let lanes = dst.len() & !3;
    ...
    unsafe {
        ...
        while i < lanes8 {
            ...
            _mm512_storeu_si512(
                ...
                _mm512_xor_si512(current0, mul0),
            );
            _mm512_storeu_si512(
                dst.as_mut_ptr().add(i + 4) as *mut __m512i,
                _mm512_xor_si512(current1, mul1),
            );
            i += 8;
        }
        let lanes = dst.len() & !3;
