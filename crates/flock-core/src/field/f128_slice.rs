...
mod tests {
    ...
    /// The optional next-slab prefetch is nonarchitectural, so both its absent
    /// and present forms have the same oracle.
    #[cfg(all(
        target_arch = "x86_64",
        target_feature = "avx512f",
        target_feature = "vpclmulqdq"
    ...
    fn fold16_banked_two_tables_matches_serial_oracle() {
        ...
        for next_slab in [None, Some(next_src.as_slice())] {
...
pub(crate) fn fold16_banked_two_tables(
    ...
    if let Some(slab) = next_slab {
        assert_eq!(slab.len(), src.len());
    }
    // SAFETY: the cfg gate supplies the CPU features; the checks above supply
    // every source, destination, table, and optional prefetch-slab bound used
    // by the four-slot body.
    unsafe {
        x86_64::fold16_banked_two_tables(
            src, f_dst, w, eq0, table0, eq1, table1, b_dst, next_slab,
        );
    }
}
