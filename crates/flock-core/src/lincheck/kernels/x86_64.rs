...
/// `build_sum_table`'s `T[1 << j] = eq8[j]`).
///
/// Built in-register as two 8×64 bit-transposes (lo / hi limbs) with direct
/// ZMM loads, byte permute, affine bit-transpose, and in-register qword byte-reversal.
/// Zero intermediate stack buffers, zero scalar loops. Bit-identical to
/// `fold_mats_from_basis_matches_sum_table`.
#[cfg(all(
    target_arch = "x86_64",
    target_feature = "avx512f",
...
///
/// 4-way unrolled across byte planes to saturate Sapphire Rapids execution
/// ports 0 and 5 concurrently with 4 independent accumulator chains.
///
