...
pub(crate) unsafe fn gather_transpose_stripe_x86(z_ptr: *const F128, stride: usize, out: *mut u8) {
    ...
    unsafe {
        ...
        // Fused lo/hi split + 8x8 byte transpose in ONE VPERMT2B per half:
        // the old permutex2var (lo/hi qword split across z0|z1) composed with
        // the 8x8 vpermb byte transpose is exactly a byte permutation of the
        // 128-byte concat(z0, z1), expressed by a static 128-byte index.
        // Bit-identical: verified against the two-instruction form by
        // gather_transpose_stripe_matches_scalar.
        #[repr(C, align(64))]
        struct FusedIdx([u8; 64]);
        static FLO: FusedIdx = {
            let mut t = [0u8; 64];
            let mut i = 0;
            while i < 64 {
        ...
        if super::super::lincheck_gt_fuse_enabled() {
            let f_lo = _mm512_load_si512(FLO.0.as_ptr() as *const __m512i);
            let f_hi = _mm512_load_si512(FHI.0.as_ptr() as *const __m512i);
            let t_lo = _mm512_gf2p8affine_epi64_epi8::<0>(
                ident,
                _mm512_permutex2var_epi8(z0, f_lo, z1),
            );
            let t_hi = _mm512_gf2p8affine_epi64_epi8::<0>(
                ident,
                _mm512_permutex2var_epi8(z0, f_hi, z1),
            );
