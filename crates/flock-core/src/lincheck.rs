...
fn take_gfni_planes(len: usize) -> Vec<u8> {
    if let Ok(mut pool) = GFNI_PLANES_POOL.lock()
        && pool.capacity() >= len
    {
        let mut planes = std::mem::take(&mut *pool);
        planes.resize(len, 0);
        return planes;
    }
    vec![0u8; len]
...
fn give_gfni_planes(planes: Vec<u8>) {
    if let Ok(mut pool) = GFNI_PLANES_POOL.lock()
        && pool.capacity() < planes.capacity()
    {
        *pool = planes;
    }
}
...
fn fold_block_major_gfni(
    ...
    let live_plane_bytes = useful_bits.div_ceil(64) * 1024;
    debug_assert!(live_plane_bytes <= k * 16);
    planes
        .par_chunks_mut(k * 16)
        .enumerate()
        .for_each(|(worker, wplanes)| {
            ...
            // The first tile seeds every live plane from a register zero
            // idiom, so it never reads stale bytes from a recycled buffer.
            ...
            // A worker that received a tile overwrote every live plane on its
            // first visit. Dynamic claiming may leave a worker with no tile;
            // only that worker's live prefix must be reset before reduction.
            // Padding planes are never read below.
            if first_tile {
                wplanes[..live_plane_bytes].fill(0);
            }
        });

    // Cross-worker reduce + transpose-back in one pass over live 64-column
    // blocks. Columns after `useful_bits` are protocol padding and therefore
    // exact zero; do not clear or merge their recycled plane storage.
    let mut out = crate::alloc_uninit_f128_vec(k);
    let live_output_len = useful_bits.div_ceil(64) * 64;
    debug_assert!(live_output_len <= k);
    let (live_out, padding_out) = out.split_at_mut(live_output_len);
    live_out.par_chunks_mut(64).enumerate().for_each(|(blk, o)| {
        let base = blk * 1024;
