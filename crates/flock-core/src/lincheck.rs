...
fn fold_block_major_gfni(
    ...
    out.par_chunks_mut(64).enumerate().for_each(|(blk, o)| {
        ...
        // The plane rows are contiguous, while the old per-column loop made
        // 16 strided byte loads for every F128. Transpose eight 8-byte groups
        // with GPR delta-swaps so every load is a contiguous u64.
        for group in 0..8 {
