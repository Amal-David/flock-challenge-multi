...
fn fold_block_major_gfni(
    ...
    planes
        ...
        .for_each(|(worker, wplanes)| {
            ...
            // Grouped-gather (four 128-bit chunks per visit) also resolved
            // once per worker — never inside the tile / chunk loops.
            #[cfg(target_feature = "avx512vbmi")]
            let gather4_fused = lc_gather4_enabled();
            ...
            loop {
                ...
                #[cfg(target_feature = "avx512vbmi")]
                if gather_tr_fused && gather4_fused {
