...
fn lincheck_gt_fuse_enabled() -> bool {
    static ON: std::sync::LazyLock<bool> = std::sync::LazyLock::new(|| {
        !lincheck_gt_fuse_disabled_value(
...
mod tests {
    ...
    fn gather_transpose_vpermt2b_selector_oracle() {
        ...
        for (high, indices) in [(false, &lo), (true, &hi)] {
            ...
            assert_eq!(indices.iter().filter(|&&index| index & 0x40 != 0).count(), 32);
            for (i, &index) in indices.iter().enumerate() {
                ...
                assert_eq!(index & 0x3f, want_offset as u8, "half={high} i={i} row={row}");
            }
        }
        ...
        for column in 0..4 {
            ...
            got[base + 64..base + 128]
                .copy_from_slice(&permute2(&a4[column], &hi, &b4[column]));
            want[base..base + 64].copy_from_slice(&direct(&a4[column], &b4[column], false));
            want[base + 64..base + 128]
                .copy_from_slice(&direct(&a4[column], &b4[column], true));
        }
        assert_eq!(got, want);
        for column in 0..4 {
            ...
            for q in [0usize, stride.saturating_sub(4).min(3)] {
                ...
                for fuse in [false, true] {
                    ...
                    for c in 0..4 {
                        ...
                        assert!(got[c * 1024 + 128..(c + 1) * 1024]
                            .iter()
                            .all(|&byte| byte == 0x5A));
                    }
                }
