...
/// `FLOCK_NO_LC_RECON_AVX512=1` restores the scalar byte-insert reconstruct
/// of 64 F128 columns from a 1 KiB plane slab. Default: the AVX-512
/// plane-to-F128 transpose (same layout as round-1 `c_plane_bank_to_f128`).
#[cfg(all(
    target_arch = "x86_64",
    target_feature = "avx512f",
    target_feature = "avx512bw",
...
fn lc_recon_avx512_enabled() -> bool {
    static ON: std::sync::LazyLock<bool> =
        std::sync::LazyLock::new(|| std::env::var_os("FLOCK_NO_LC_RECON_AVX512").is_none());
    *ON
}
...
fn fold_block_major_gfni(
    ...
    let reconstruct = |acc: &[u8], o: &mut [F128]| {
        ...
        if lc_recon_avx512_enabled() {
            // SAFETY: 16×64 plane slab and 64 F128 outputs; layout is
            // `acc[byte * 64 + col]`, the same as `c_plane_bank_to_f128`.
            unsafe {
                kernels::planes_to_f128_avx512(acc.as_ptr(), o.as_mut_ptr());
            }
            return;
        }
