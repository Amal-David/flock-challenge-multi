    ...
    fn fold_alpha_batched(&self, alpha: F128, eq_inner: &[F128]) -> Vec<F128> {
        ...
        if self.narrow {
            ...
            return self.map_cols(|c| {
                let a_slice = &self.a_rows16[self.a_col_ptr[c] as usize..self.a_col_ptr[c + 1] as usize];
                let mut sa_lo = 0u64;
                let mut sa_hi = 0u64;
                let mut i = 0;
                while i + 4 <= a_slice.len() {
                    let r0 = a_slice[i] as usize;
                    let r1 = a_slice[i + 1] as usize;
                    let r2 = a_slice[i + 2] as usize;
                    let r3 = a_slice[i + 3] as usize;
                    // SAFETY: r < n_rows ≤ eq_inner.len(), asserted above.
                    let e0 = *unsafe { eq_inner.get_unchecked(r0) };
                    let e1 = *unsafe { eq_inner.get_unchecked(r1) };
                    let e2 = *unsafe { eq_inner.get_unchecked(r2) };
                    let e3 = *unsafe { eq_inner.get_unchecked(r3) };
                    sa_lo ^= e0.lo ^ e1.lo ^ e2.lo ^ e3.lo;
                    sa_hi ^= e0.hi ^ e1.hi ^ e2.hi ^ e3.hi;
                    i += 4;
                }
                while i < a_slice.len() {
                    let r = a_slice[i] as usize;
                    let e = *unsafe { eq_inner.get_unchecked(r) };
                    sa_lo ^= e.lo;
                    sa_hi ^= e.hi;
                    i += 1;
                }
                let sa = F128 { lo: sa_lo, hi: sa_hi };

                let b_slice = &self.b_rows16[self.b_col_ptr[c] as usize..self.b_col_ptr[c + 1] as usize];
                let mut sb_lo = 0u64;
                let mut sb_hi = 0u64;
                let mut j = 0;
                while j + 4 <= b_slice.len() {
                    let r0 = b_slice[j] as usize;
                    let r1 = b_slice[j + 1] as usize;
                    let r2 = b_slice[j + 2] as usize;
                    let r3 = b_slice[j + 3] as usize;
                    // SAFETY: as above.
                    let e0 = *unsafe { eq_inner.get_unchecked(r0) };
                    let e1 = *unsafe { eq_inner.get_unchecked(r1) };
                    let e2 = *unsafe { eq_inner.get_unchecked(r2) };
                    let e3 = *unsafe { eq_inner.get_unchecked(r3) };
                    sb_lo ^= e0.lo ^ e1.lo ^ e2.lo ^ e3.lo;
                    sb_hi ^= e0.hi ^ e1.hi ^ e2.hi ^ e3.hi;
                    j += 4;
                }
                while j < b_slice.len() {
                    let r = b_slice[j] as usize;
                    let e = *unsafe { eq_inner.get_unchecked(r) };
                    sb_lo ^= e.lo;
                    sb_hi ^= e.hi;
                    j += 1;
                }
                let sb = F128 { lo: sb_lo, hi: sb_hi };
                alpha * sa + sb
            });
        }
        self.map_cols(|c| {
            let a_slice = &self.a_rows[self.a_col_ptr[c] as usize..self.a_col_ptr[c + 1] as usize];
            let mut sa_lo = 0u64;
            let mut sa_hi = 0u64;
            let mut i = 0;
            while i + 4 <= a_slice.len() {
            ...
            let b_slice = &self.b_rows[self.b_col_ptr[c] as usize..self.b_col_ptr[c + 1] as usize];
            let mut sb_lo = 0u64;
            let mut sb_hi = 0u64;
            let mut j = 0;
            while j + 4 <= b_slice.len() {
