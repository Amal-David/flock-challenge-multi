        ...
        .for_each(|(worker, partial)| {
            ...
            loop {
                ...
                for q in 0..useful_chunks {
                    ...
                    #[cfg(target_arch = "x86_64")]
                    let transposed_done = if tile_stripes == DIRECT_FOLD_TILE_STRIPES {
                        // SAFETY: same index contract as the aarch64
                        // `gather_transpose_tile_neon` arm above. Full-tile
                        // only: the last partial tile still takes the scalar
                        // loop. AVX-512 i64gather of the 8 lo/hi u64s replaces
                        // the scalar 2 KiB-strided load (SPR L2-hostile).
                        unsafe {
