...
fn compute_combined_basis_and_target<Ch: Challenger>(
    ...
    let ((mut round0_u0, mut round0_u2), mut round1_lookahead) =
        in_wide_combine_pool(l, || if use_direct_fold8 || use_direct_fold4 {
        // All six/four initial messages come from the two claims' retained
        // product matrices below; no L-sized basis exists to sweep.
