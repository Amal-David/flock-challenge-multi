...
fn in_ligerito_physical_pool<R: Send>(_l: usize, op: impl FnOnce() -> R + Send) -> R {
    // Candidate isolation: retain the three sparse-induction transformations
    // but keep the promoted logical-thread placement for recursive Ligerito.
    op()
}
...
fn in_ring_switch_physical_pool<R: Send>(_l: usize, op: impl FnOnce() -> R + Send) -> R {
    // Candidate isolation: ring switch stays on the promoted global pool.
    op()
}
