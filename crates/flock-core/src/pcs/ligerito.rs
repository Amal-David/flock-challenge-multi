...
/// Profile shape derivation: a `first_recursive_k`-bit fold followed by
/// `recursive_k`-bit folds, with the rate index increasing by at least one per
/// level and bumped further whenever the block length cannot accommodate
/// `queries_at_rate(rate)` distinct queries.
fn derive_ladder_shape(
    log_n: usize,
    initial_k: usize,
    log_inv_rate: usize,
    first_recursive_k: usize,
    recursive_k: usize,
    ...
    while n_running > 5 {
        let requested_k = if shape.k_recursive.len() == 1 {
            first_recursive_k
        } else {
            recursive_k
        };
        let k = requested_k.min(n_running);
        let log_msg_cols_next = n_running - k;
        let mut next_rate = rate_running + 1;
        loop {
...
impl LigeritoSecurityConfig {
    ...
    ///
    /// - `Fast`:   JohnsonOod, rate 1/2, η = 0.04, a 9-bit first recursive
    ///             fold followed by 7-bit folds, 100 bits per round.
    ...
    pub fn derive_profile(m: usize, profile: LigeritoProfile) -> Result<Self, String> {
        ...
        let (first_recursive_k, recursive_k) = match profile {
            LigeritoProfile::Fast => (9, 7),
            LigeritoProfile::Slim | LigeritoProfile::Secure => (3, 3),
        };
...
mod tests {
    ...
    /// parse and validate. Fast uses its tuned 9+7-bit ladder; Slim retains the
    ...
    fn ligerito_security_config_m29_toml_loads() {
        ...
        assert_eq!(pv.recursive_ks, vec![9, 7]);
