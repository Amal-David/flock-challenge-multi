...
fn fold_and_msg_lsb_inner(
    ...
    let (u_0, u_2) = nf_s
        ...
        .map(|(ci, (fc, bc))| {
            ...
            for (g_b, g_alpha) in glues {
                let mut temp = [F128::ZERO; CHUNK];
                let temp_slice = &mut temp[..len];
                crate::field::f128_slice::fold_pairs(g_b, base, temp_slice, r);
                crate::field::f128_slice::add_scaled(bc, temp_slice, *g_alpha);
            }
            if let Some((eq_lo, eq_hi, gamma)) = lazy_ood {
