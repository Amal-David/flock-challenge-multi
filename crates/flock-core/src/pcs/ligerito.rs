...
unsafe fn msg_reduce_eval_avx512(fc: &[F128], bc: &[F128]) -> (F128, F128, F128) {
    ...
    unsafe {
        let mut u0_acc = WideGhashX4::zero();
        let mut u2_acc = WideGhashX4::zero();
        // `u0_acc` already owns every even-slot product.  Accumulate only the
        // complementary odd slots here, then combine the two reduced sums.
        // This removes one four-lane carry-less multiply from every 8 inputs.
        ...
        let mut u0 = u0_acc.fold().reduce();
        let mut u2 = u2_acc.fold().reduce();
        let mut y = u0 + odd_acc.fold().reduce();
