...
/// Same-binary rollback for pinning the open/induce overlap onto the
/// round-1 SMT half-pools (sibling 0 of every physical core vs sibling 1).
/// Those pools already exist on the ranked 8-core+SMT shape; reusing them
/// here pairs a DRAM gather (rows + Merkle) with a CLMUL Fᵀ-NTT induce the
/// same way round 1 pairs AB with identity-C. `FLOCK_NO_LIG_OPEN_SMT_SPLIT=1`
/// restores the incumbent `rayon::join` on the global pool.
fn open_smt_split_enabled() -> bool {
    static ON: std::sync::LazyLock<bool> = std::sync::LazyLock::new(|| {
        std::env::var_os("FLOCK_NO_LIG_OPEN_SMT_SPLIT").is_none()
    });
    *ON
}

/// Run two independent producers. When the ranked half-pools exist and the
/// SMT-split selector is on, each producer is installed on one sibling
/// group; otherwise this is exactly `rayon::join`. Schedule only: both
/// closures are identical to the incumbent arms.
fn open_smt_join<A, B, RA, RB>(left: A, right: B) -> (RA, RB)
where
    A: FnOnce() -> RA + Send,
    B: FnOnce() -> RB + Send,
    RA: Send,
    RB: Send,
{
    match crate::smt_split::zc_r1_pools() {
        Some((left_pool, right_pool)) if open_smt_split_enabled() => {
            rayon::join(|| left_pool.install(left), || right_pool.install(right))
        }
        _ => rayon::join(left, right),
    }
}
...
mod tests {
    ...
    /// `open_smt_join` is the ranked L0/L1 overlap dispatcher. Drive it on
    /// the shipped induce functions: one arm is `induce_sumcheck_poly_via_ntt`,
    /// the other is the dense `induce_sumcheck_poly`. Both must match each
    /// other and the serial calls. On this host the half-pools may be absent
    /// (the helper then is `rayon::join`); on the ranked 8+SMT shape the
    /// same closures run on sibling 0 vs sibling 1.
    #[test]
    fn open_smt_join_drives_shipped_induce_arms() {
        use crate::challenger::Challenger;
        let (log_msg, log_inv_rate, log_int, n_queries) = (10usize, 1usize, 6usize, 218usize);
        let block_len = 1usize << (log_msg + log_inv_rate);
        let num_interleaved = 1usize << log_int;
        let mut ch = crate::challenger::RandomChallenger::new(0x5E17_1011);
        let mut queries: Vec<usize> = Vec::new();
        while queries.len() < n_queries.min(block_len) {
            let q = (ch.sample_f128().lo as usize) % block_len;
            if !queries.contains(&q) {
        ...
        let alpha = ch.sample_f128_vec(ceil_log2(nq.max(1)));
        let sks_vks = eval_sk_at_vks(log_msg);
        let dense_serial = induce_sumcheck_poly(
            log_msg,
            &sks_vks,
            &opened_rows,
            &v_challenges,
        ...
        let ntt_serial = induce_sumcheck_poly_via_ntt(
            log_msg,
            log_inv_rate,
            &opened_rows,
            &v_challenges,
        ...
        let (ntt_joined, dense_joined) = open_smt_join(
            || {
                induce_sumcheck_poly_via_ntt(
                    log_msg,
                    log_inv_rate,
        ...
        assert_eq!(ntt_joined, ntt_serial);
        assert_eq!(dense_joined, dense_serial);
        assert_eq!(ntt_joined.0, dense_joined.0);
        assert_eq!(ntt_joined.1, dense_joined.1);
    }
