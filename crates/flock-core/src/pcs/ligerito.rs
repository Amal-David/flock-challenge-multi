...
static MSG_REDUCE_PIPE_TEST_OFF: std::sync::atomic::AtomicBool =
    std::sync::atomic::AtomicBool::new(false);

/// `FLOCK_NO_MSG_REDUCE_PIPE=1` restores the serial 8-wide load/mul_acc
/// loop. Default ON: preload the next 8 F128 while `mul_acc` of the
/// current chunk covers CLMUL latency. Ranked `env_clear()`.
#[cfg(all(target_feature = "avx512f", target_feature = "vpclmulqdq"))]
fn msg_reduce_pipe_enabled() -> bool {
    #[cfg(test)]
    if MSG_REDUCE_PIPE_TEST_OFF.load(std::sync::atomic::Ordering::Relaxed) {
        return false;
    }
    static ON: std::sync::LazyLock<bool> =
        std::sync::LazyLock::new(|| std::env::var_os("FLOCK_NO_MSG_REDUCE_PIPE").is_none());
    *ON
}
...
pub(crate) unsafe fn msg_reduce_avx512(fc: &[F128], bc: &[F128]) -> (F128, F128) {
    ...
    let load = |k: usize| unsafe {
        (
            _mm512_loadu_si512(fc.as_ptr().add(k) as *const __m512i),
            _mm512_loadu_si512(fc.as_ptr().add(k + 4) as *const __m512i),
            _mm512_loadu_si512(bc.as_ptr().add(k) as *const __m512i),
            _mm512_loadu_si512(bc.as_ptr().add(k + 4) as *const __m512i),
        )
    };
    let acc_chunk = |u0: &mut WideGhashX4,
                     u2: &mut WideGhashX4,
                     f0: __m512i,
                     f1: __m512i,
                     b0: __m512i,
        ...
        u2.mul_acc(f_sum, b_sum);
    };
    if msg_reduce_pipe_enabled() && lanes >= 16 {
        let (mut f0, mut f1, mut b0, mut b1) = load(0);
        while k + 16 <= lanes {
            let (nf0, nf1, nb0, nb1) = load(k + 8);
            acc_chunk(&mut u0_acc, &mut u2_acc, f0, f1, b0, b1);
            f0 = nf0;
            f1 = nf1;
            b0 = nb0;
            b1 = nb1;
    ...
    unsafe {
        ...
        if msg_reduce_pipe_enabled() && lanes >= 16 {
            ...
            while k + 16 <= lanes {
                let (nf0, nf1, nb0, nb1) = load(k + 8);
                acc_chunk(
                    &mut u0_acc, &mut u2_acc, &mut y_acc, f0, f1, b0, b1,
                );
                f0 = nf0;
                f1 = nf1;
                b0 = nb0;
...
mod tests {
    ...
    /// Piped 8-wide msg_reduce must match the serial load/mul_acc body.
    #[cfg(all(target_feature = "avx512f", target_feature = "vpclmulqdq"))]
    #[test]
    fn msg_reduce_pipe_matches_serial() {
        use std::sync::atomic::Ordering;
        let mut state = 0x5EED_F01D_u64;
        let mut next = || {
            state ^= state << 13;
            state ^= state >> 7;
            state ^= state << 17;
        ...
        for &n in &[0usize, 1, 2, 7, 8, 9, 15, 16, 17, 32, 64, 128, 255] {
            let fc: Vec<F128> = (0..n).map(|_| next()).collect();
            let bc: Vec<F128> = (0..n).map(|_| next()).collect();
            let run = |off: bool| {
                MSG_REDUCE_PIPE_TEST_OFF.store(off, Ordering::Relaxed);
                let got = unsafe { msg_reduce_avx512(&fc, &bc) };
                MSG_REDUCE_PIPE_TEST_OFF.store(false, Ordering::Relaxed);
                got
            };
            let piped = run(false);
            let serial = run(true);
            assert_eq!(piped, serial, "n={n}");
        }
    }
