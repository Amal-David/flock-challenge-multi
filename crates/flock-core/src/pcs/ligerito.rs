...
unsafe fn transpose_butterfly_avx512(top: &mut [F128], bot: &mut [F128], t: F128) {
    ...
    unsafe {
        ...
        let lanes = top.len() & !3;
        let mut i = 0;
        // Two independent 4-lane chunks cover CLMUL latency the way NTT
        // fused-3 already does (`butterfly2`). `FLOCK_NO_TNT_BUTTERFLY_X8=1`
        // restores the one-chunk loop. Ranked env_clear so default ON.
        if tnt_butterfly_x8_enabled() {
            while i + 8 <= lanes {
                let va0 = _mm512_loadu_si512(top.as_ptr().add(i) as *const __m512i);
                let vb0 = _mm512_loadu_si512(bot.as_ptr().add(i) as *const __m512i);
                let va1 = _mm512_loadu_si512(top.as_ptr().add(i + 4) as *const __m512i);
                let vb1 = _mm512_loadu_si512(bot.as_ptr().add(i + 4) as *const __m512i);
                let vs0 = _mm512_xor_si512(va0, vb0);
                let vs1 = _mm512_xor_si512(va1, vb1);
                _mm512_storeu_si512(top.as_mut_ptr().add(i) as *mut __m512i, vs0);
                _mm512_storeu_si512(top.as_mut_ptr().add(i + 4) as *mut __m512i, vs1);
                let nb0 = _mm512_xor_si512(vb0, ghash_mul_x4_split(vs0, tb, tb_x64));
                let nb1 = _mm512_xor_si512(vb1, ghash_mul_x4_split(vs1, tb, tb_x64));
                _mm512_storeu_si512(bot.as_mut_ptr().add(i) as *mut __m512i, nb0);
                _mm512_storeu_si512(bot.as_mut_ptr().add(i + 4) as *mut __m512i, nb1);
                i += 8;
            }
        }
        while i < lanes {
...
/// Test-only latch so the x8 butterfly oracle can drive both arms without
/// mutating process env.
#[cfg(all(test, target_feature = "avx512f", target_feature = "vpclmulqdq"))]
static TNT_X8_TEST_OFF: std::sync::atomic::AtomicBool = std::sync::atomic::AtomicBool::new(false);

/// `FLOCK_NO_TNT_BUTTERFLY_X8=1` restores the one-chunk 4-lane transpose
/// butterfly. Default ON: two independent 4-lane chunks per iteration cover
/// CLMUL latency. Ranked `env_clear()`. Prefetch/algebra unchanged.
#[cfg(all(target_feature = "avx512f", target_feature = "vpclmulqdq"))]
fn tnt_butterfly_x8_enabled() -> bool {
    #[cfg(test)]
    if TNT_X8_TEST_OFF.load(std::sync::atomic::Ordering::Relaxed) {
        return false;
    }
    static ON: std::sync::LazyLock<bool> =
        std::sync::LazyLock::new(|| std::env::var_os("FLOCK_NO_TNT_BUTTERFLY_X8").is_none());
    *ON
}
...
mod tests {
    ...
    /// Two-chunk transpose butterfly must match the one-chunk 4-lane body
    /// on the shipped [`transpose_butterfly_avx512`].
    #[cfg(all(target_feature = "avx512f", target_feature = "vpclmulqdq"))]
    #[test]
    fn transpose_butterfly_x8_matches_x4() {
        use std::sync::atomic::Ordering;
        let mut state = 0x7E70_0008_u64;
        let mut next = || {
            state ^= state << 13;
            state ^= state >> 7;
            state ^= state << 17;
        ...
        for &n in &[1usize, 3, 4, 5, 7, 8, 9, 15, 16, 64, 128] {
            let top0: Vec<F128> = (0..n).map(|_| next()).collect();
            let bot0: Vec<F128> = (0..n).map(|_| next()).collect();
            let t = next();
            let run = |off: bool| {
                let mut top = top0.clone();
                let mut bot = bot0.clone();
                TNT_X8_TEST_OFF.store(off, Ordering::Relaxed);
                unsafe {
                    transpose_butterfly_avx512(&mut top, &mut bot, t);
                }
                TNT_X8_TEST_OFF.store(false, Ordering::Relaxed);
                (top, bot)
            };
            let x8 = run(false);
            let x4 = run(true);
            assert_eq!(x8, x4, "n={n} t={t:?}");
        }
    }
