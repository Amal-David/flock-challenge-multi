...
/// Process-local inverses of the canonical standard-basis values returned by
/// [`eval_sk_at_vks`]. Callers must pass that canonical vector for the matching
/// dimension. The ranked worker runs a same-shape warm-up proof before readiness,
/// so scored proofs hit only the initialized `OnceLock` cells.
#[inline]
fn cached_inv_sks_vks(sks_vks: &[F128]) -> &'static [F128] {
    static CACHE: [std::sync::OnceLock<Vec<F128>>; u64::BITS as usize] =
        [const { std::sync::OnceLock::new() }; u64::BITS as usize];

    let log_n = sks_vks
        .len()
        .checked_sub(1)
        .expect("sks_vks must contain s_0(v_0)");
    debug_assert_eq!(
        sks_vks,
        eval_sk_at_vks(log_n).as_slice(),
        "cached inverses require canonical eval_sk_at_vks input",
    );

    CACHE
        .get(log_n)
        .expect("LCH log_n exceeds the standard F128 basis")
        .get_or_init(|| {
            sks_vks
                .iter()
...
pub(crate) fn induce_sumcheck_evaluate_at_residual(
    ...
    let inv_sks_vks = cached_inv_sks_vks(sks_vks);
