...
/// its final dense slot. This removes the per-window result allocation and
/// the following copy. Independent of local-NTT fusion: fused local layers
/// still require `induce_densify_local_ntt_fuse_enabled`.
...
fn transpose_forward_ntt_sparse_inner(
    ...
    // Direct-into-slot expansion does not require local-NTT fusion: it only
...
mod tests {
    ...
    /// Ranked path: `densify_sparse_runs` (direct-into-slot, no local-NTT
    /// fusion) must be byte-identical to the processed-window expand kernels
    /// plus `densify_windows_fused`. This is the arm the empty ranked env
    /// actually takes after splitting `direct_windows` off of
    /// `local_ntt_fused`. Covers singleton, multi-hit (≤ k/2), dense-window
    /// fallback, inactive slots, and the ranked L0 (log_d=20, k=8) shape.
    #[test]
    fn densify_sparse_runs_matches_processed_windows() {
        use crate::challenger::Challenger;
        for &(log_d, k, n_hits) in &[
            (12usize, 8usize, 40usize),
            (16, 8, 80),
            (18, 8, 120),
            (20, 8, 214),
            (16, 10, 50),
            ...
            let ntt = AdditiveNttF128::standard(log_d);
            let mut ch = crate::challenger::RandomChallenger::new(
                0xD1EC_7000 ^ ((log_d as u64) << 16) ^ (k as u64) << 8 ^ n_hits as u64,
            );
            let mut positions: Vec<usize> = Vec::new();
            while positions.len() < n_hits.min(n / 4).max(1) {
                let p = (ch.sample_f128().lo as usize) % n;
                if !positions.contains(&p) {
                    positions.push(p);
                }
            ...
            for &(rs, re) in &runs {
                let window = positions[order[rs] as usize] >> k;
                assert!(run_slots[window].is_none());
                run_slots[window] = Some((rs, re));
            }

            let got = densify_sparse_runs(
                &ntt,
                n,
                k,
                &positions,
                ...
                if nnz <= multi_max {
                    ...
                    if nnz > 1 {
                        ...
                        for &i in &order[rs + 1..re] {
                            ...
                            for (d, &extra) in buf.iter_mut().zip(scratch.iter()) {
                                *d += extra;
                            }
                        }
                    }
                    processed.push((w, buf));
                } else {
                    let mut buf = vec![F128::ZERO; 1 << k];
                    for &i in &order[rs..re] {
                        let i = i as usize;
                    ...
                    processed.push(transpose_forward_ntt_window_dense(
                        &ntt, log_d, k, w, buf,
                    ));
                }
            }
            let want = densify_windows_fused(n, k, window_slots(nwin, processed));
            assert_eq!(
                got, want,
                "densify_sparse_runs != processed+fused log_d={log_d} k={k} hits={}",
                positions.len()
            );

            // Full remaining dense layers: the ranked second pass.
            let mut got_full = got;
            let mut want_full = want;
            transpose_forward_ntt_dense_layers(&ntt, &mut got_full, log_d - k);
            transpose_forward_ntt_dense_layers(&ntt, &mut want_full, log_d - k);
            assert_eq!(
                got_full, want_full,
                "post-dense-layers mismatch log_d={log_d} k={k}"
            );
        }
    }
