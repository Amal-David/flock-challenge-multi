...
/// Compute the introduce-round message against `f` while the just-induced
/// basis is still the producer’s write, instead of after `RecursiveProof`
/// assembly has cooled it. Algebra is [`round_msg_lsb`]; `introduce_new`
/// then skips its own pass. `FLOCK_NO_LIG_INDUCE_FUSED_MSG=1` restores the
/// post-assembly pass. Ranked worker env is cleared, so default ON.
fn lig_induce_fused_msg_enabled() -> bool {
    static ON: std::sync::LazyLock<bool> =
        std::sync::LazyLock::new(|| std::env::var_os("FLOCK_NO_LIG_INDUCE_FUSED_MSG").is_none());
    *ON
}
...
impl SumcheckProver {
    ...
    /// [`Self::introduce_new`] with a caller-supplied [`round_msg_lsb`] result.
    /// Used when that pass already ran against this `b_new` (induce fusion).
    /// Transcript-identical to `introduce_new` on the same `(f, b_new, h_new, msg)`.
    pub fn introduce_new_with_msg(
        &mut self,
        b_new: Vec<F128>,
        h_new: F128,
        msg: SumcheckMessage,
...
fn recursive_prover_with_basis_impl<Ch: Challenger>(
    ...
    // Fuse the introduce message here, while `basis_0_induced` is still the
    // induce producer’s write. The RecursiveProof move below is the cooling
    // window the unfused `introduce_new` pass used to re-read through.
    let fused_msg_0 =
        lig_induce_fused_msg_enabled().then(|| round_msg_lsb(sc_prover.f(), &basis_0_induced));
    ...
    let _t = std::time::Instant::now();
    let intro_msg_0 = match fused_msg_0 {
        Some(msg) => sc_prover.introduce_new_with_msg(basis_0_induced, enforced_sum_0, msg),
        None => sc_prover.introduce_new(basis_0_induced, enforced_sum_0),
    };
    ...
    for i in 0..r {
        ...
        let fused_msg_i =
            lig_induce_fused_msg_enabled().then(|| round_msg_lsb(sc_prover.f(), &basis_i_induced));
        ...
        let _t = std::time::Instant::now();
        let intro_msg_i = match fused_msg_i {
            Some(msg) => sc_prover.introduce_new_with_msg(basis_i_induced, enforced_sum_i, msg),
            None => sc_prover.introduce_new(basis_i_induced, enforced_sum_i),
        };
...
mod tests {
    ...
    /// [`SumcheckProver::introduce_new_with_msg`] is the induce-fusion
    /// entry: same transcript as [`SumcheckProver::introduce_new`] when the
    /// supplied message is [`round_msg_lsb`] of the same `(f, b_new)`.
    #[test]
    fn introduce_new_with_msg_matches_introduce_new() {
        use crate::challenger::Challenger;
        for &log_n in &[4usize, 8, 12] {
            let mut ch = crate::challenger::RandomChallenger::new(0xF05E_0001 ^ log_n as u64);
            let n = 1usize << log_n;
            let f: Vec<F128> = (0..n).map(|_| ch.sample_f128()).collect();
            let b1: Vec<F128> = (0..n).map(|_| ch.sample_f128()).collect();
            let b_new: Vec<F128> = (0..n).map(|_| ch.sample_f128()).collect();
            let h1 = ch.sample_f128();
            let h_new = ch.sample_f128();
            let (mut a, start_a) = SumcheckProver::new(f.clone(), b1.clone(), h1);
            let (mut b, start_b) = SumcheckProver::new(f, b1, h1);
            assert_eq!(start_a, start_b);
            let want = a.introduce_new(b_new.clone(), h_new);
            let msg = round_msg_lsb(b.f(), &b_new);
            let got = b.introduce_new_with_msg(b_new, h_new, msg);
            assert_eq!(got, want, "log_n={log_n}");
            assert_eq!(b.transcript(), a.transcript(), "log_n={log_n}");
        }
    }
