...
fn fold_and_msg_lsb_inner(
    ...
    let use_soa = lazy_ood.is_none() && cfg!(target_feature = "sha3") && {
        static SOA: std::sync::LazyLock<bool> =
            std::sync::LazyLock::new(|| std::env::var_os("FLOCK_NO_OPEN_SUMCHECK_OPT").is_none());
        *SOA
    };
...
fn materialize_direct_fold4(
    ...
    // Ranked pin16 orders the first sibling of all eight cores first, then
    // their second siblings. Splitting at eight puts opposite phases on each
    // physical core; other pool shapes keep the incumbent f-first order.
    let stagger_split =
        (cfg!(target_arch = "x86_64") && rayon::current_num_threads() == 16).then_some(8);
    let (u_0, u_2) = folded_b
        ...
        .map_init(
            ...
            |(scratch, mid), (block, (b_out, f_out))| {
                ...
                let b_first = stagger_split.is_some_and(|split| {
                    rayon::current_thread_index().is_some_and(|index| index >= split)
                });
                let f_in = &packed_witness[start..start + 16 * block_len];
                // The b fold prefetches the slab this worker will fold next:
                // its current slab for b-first siblings, or the next block for
                // incumbent-order siblings. The current slab is `f_in`; the
                // next slab is in-bounds exactly when `block + 1 < n_blocks`.
                #[cfg(target_arch = "x86_64")]
                let (pf_base, pf_span) = if !pf_on {
                    ...
                    // SAFETY: `16·(block+2)·block_len` is at most the packed
                    // witness length, so the complete next slab is in-bounds.
                ...
                let mut fold_f = |mid: &mut Vec<F128>| {
                if deferred_reduce {
                ...
                let mut fold_b = |mid: &mut Vec<F128>| {
                ...
                // Execute opposite orders on the two pinned siblings of every
                // physical core. Both outputs are disjoint and the message
                // reduction below remains after both operations.
                if b_first {
                    fold_b(mid);
                    fold_f(mid);
                } else {
...
impl SumcheckProver {
    ...
    pub fn fold(&mut self, r: F128) -> SumcheckMessage {
        ...
        let (nf, nb, msg) = match pending_ood {
            ...
            None => fold_and_msg_lsb(&self.f, &self.combined_basis, r, self.fold_arena.as_mut()),
        };
