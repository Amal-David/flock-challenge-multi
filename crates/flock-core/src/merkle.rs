...
pub(crate) fn hash_leaves(data: &[u8], leaf_size: usize, out: &mut [Hash], kind: HashKind) {
    ...
    match kind {
        HashKind::Blake3 if blake3_leaf_size_is_batchable(leaf_size) => {
            let group = (out.len() / 32).next_power_of_two().clamp(64, BLAKE3_GROUP);
            out.par_chunks_mut(group)
                .zip(data.par_chunks(group * leaf_size))
...
pub(crate) fn hash_pairs_level(read: &[Hash], write: &mut [Hash], kind: HashKind) {
    ...
    let read_bytes: &[u8] =
        unsafe { core::slice::from_raw_parts(read.as_ptr() as *const u8, read.len() * 32) };
    const SERIAL_LEVEL_NODES: usize = 512;
    ...
    match kind {
        HashKind::Blake3 => {
            if serial {
                ...
                let group = (write.len() / 32).next_power_of_two().clamp(64, BLAKE3_GROUP);
                write
