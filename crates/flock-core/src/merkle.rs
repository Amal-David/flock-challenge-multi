...
///
/// Used by the production StreamMerkle coordinator so parent folding does
/// not steal NTT workers off the global pool.
...
impl StreamMerkle {
    ...
    ///
    /// Requires `lo == hashed prefix`. Production uses this from the
    /// coordinator after [`StreamMerkleInbox`] drains a contiguous completed
    /// prefix — never from `on_range_done`.
...
///
/// NTT `on_range_done` completes static sub-groups via `par_chunks_mut` and
/// may fire out of leaf-index order. Hash leaves on the publishing worker
/// (`hash_leaves_serial`), then enqueue the range here. Parents fold only
/// when a contiguous written prefix advances. Calling
/// [`StreamMerkle::absorb_leaf_range`] from that callback is a reject
...
/// contiguous written prefix (production coordinator contract).
...
mod tests {
    ...
    /// Production coordinator contract: out-of-order completed ranges still
    /// produce the same `2n−1` tree. Parents fold only on a contiguous
    /// written prefix (a hole is enqueued, not absorbed).
    #[test]
