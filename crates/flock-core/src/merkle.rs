...
///
/// Production CPU L0 uses this from the NTT worker after a retired
/// sub-group's leaf hash ([`fold_range_to_root`]): the first hop at the
/// ranked 2048-leaf grain is exactly 1024 parents, which is also
/// [`hash_pairs_level`]'s serial threshold — calling the par path from
/// inside the deep pass would oversub. Still SIMD-batched (`blake3_hash_many`
/// / `sha256_hash4` quads), never 2-parent crumbs.
...
/// Fold every complete parent pair whose both children sit in the already-hashed
/// leaf range `[lo, hi)`. Stops after writing the single **range-root**.
/// Does not pair that root with a sibling outside the range — those parents
/// wait for `build_upper_levels` after the encode join, starting at
/// `from_nodes = n_leaves / (hi - lo)`.
///
/// `hi - lo` must be a power of two and `lo` must be a multiple of that
/// length (a complete subtree in the flat `2n−1` layout). Production grain
/// is the NTT sub-group (ranked: 2048 leaves / 2 MB, `n_top=9` → 512
/// range-roots). Out-of-order `par_chunks_mut` is fine: each range owns a
/// disjoint subtree, so a later hole does not uninit this range's pairs.
///
/// Narrow raw parts only: this range's children and new parent slots.
/// Later unwritten leaves and other workers' parent slots stay unborrowed.
pub(crate) fn fold_range_to_root(
    tree: *mut Hash,
    n_leaves: usize,
    lo: usize,
    hi: usize,
    ...
    assert!(hi > lo && hi <= n_leaves);
    let group_len0 = hi - lo;
    assert!(
        group_len0.is_power_of_two() && lo % group_len0 == 0,
        "range {lo}..{hi} is not an aligned complete subtree of n={n_leaves}"
    );

    let mut s = n_leaves;
    let mut group_off = lo;
    let mut group_len = group_len0;
    while group_len > 1 {
        let child_start = 2 * n_leaves - 2 * s;
        let parent_start = child_start + s;
        let read_lo = child_start + group_off;
        let read_hi = read_lo + group_len;
        let write_lo = parent_start + group_off / 2;
        let write_hi = write_lo + group_len / 2;
        debug_assert!(read_hi <= parent_start);
        debug_assert!(write_hi <= 2 * n_leaves - 1);
        // SAFETY: `read_lo..read_hi` is this worker's already-written
        // children (leaves just hashed on this thread, or parents this
        // function just wrote). `write_lo..write_hi` are this subtree's
        // parent slots — disjoint from every other aligned `group_len0`
        // range, and NTT workers never write parent slots. The two ranges
        // are disjoint (`read_hi <= parent_start <= write_lo`) and
        // in-bounds. Later leaves and other ranges' nodes are not
        // referenced.
        let read = unsafe { core::slice::from_raw_parts(tree.add(read_lo), group_len) };
        let write = unsafe { core::slice::from_raw_parts_mut(tree.add(write_lo), group_len / 2) };
        hash_pairs_level_serial(read, write, kind);
        s >>= 1;
        group_off >>= 1;
        group_len >>= 1;
    }
...
impl StreamMerkle {
    ...
    /// L0 folds intra-range pairs via [`fold_range_to_root`] on the same
    /// NTT worker, not through this coordinator API.
...
/// and does not spawn a parent coordinator — leaves are hashed
/// same-worker, then [`fold_range_to_root`] folds intra-range pairs;
/// `build_upper_levels` finishes from the range-root spine after join.
/// Calling [`StreamMerkle::absorb_leaf_range`] from `on_range_done` is a
/// reject (`lo == prefix` plus parallel `hash_leaves`).
#[allow(dead_code)]
...
mod tests {
    ...
    /// Same-worker salvage: each aligned sub-group folds complete pairs up
    /// to its range-root (OOO order is fine — no cross-range sibling).
    /// Remaining levels start at `n_leaves / grain` (the range-root spine).
    #[test]
    fn fold_range_to_root_then_upper_matches_merkle_tree() {
        ...
        for &(n_leaves, leaf_size, grain) in cases {
            assert!(n_leaves % grain == 0 && grain.is_power_of_two());
            let data = random_data(
                n_leaves,
                leaf_size,
                0x51EA_u64.wrapping_mul(n_leaves as u64),
            ...
            for kind in KINDS {
                ...
                hash_leaves(&data, leaf_size, &mut tree[..n_leaves], kind);
                let mut starts: Vec<usize> = (0..n_leaves).step_by(grain).collect();
                starts.reverse();
                for lo in starts {
                    fold_range_to_root(tree.as_mut_ptr(), n_leaves, lo, lo + grain, kind);
                }
                let mut s = n_leaves / grain;
                while s > 1 {
                ...
                assert_eq!(
                    tree, oneshot,
                    "{kind} n={n_leaves} grain={grain}: node-for-node vs merkle_tree"
                );
                assert_eq!(
                    tree[2 * n_leaves - 2],
                    *oneshot.last().unwrap(),
                    "{kind} n={n_leaves} grain={grain}: root at 2n-2"
                );
            }
        }
    }
