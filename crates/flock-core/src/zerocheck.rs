...
/// with an optional packed a/b octa regenerator (see
/// [`multilinear::AbOctaRegen`]): when supplied, both the round-two sweep and
/// the no-materialize rounds-3+4 pass rebuild each chunk's packed a/b rows
/// into a task-local tile from the closed form instead of streaming the two
/// packed buffers from DRAM. Value-identical — the regenerated bytes equal
/// the drained bytes by construction.
#[allow(clippy::too_many_arguments)]
