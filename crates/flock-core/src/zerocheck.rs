...
/// levels now ship on, so both latches force their level *off*. Either way
/// both routes emit the same transcript — that is what the
...
/// Cascade level 4 (rounds 11+12) now ships **on**; `FLOCK_NO_ZC_CASCADE5=1`
/// stops the cascade after rounds 9+10 (the previous frontier behavior). The
/// original author measured the composed pass as a 0.4 ms regression against
/// the calling-thread tail route on an idle 16-thread Zen 5 and shipped it
/// opt-in, explicitly flagging that rayon fan-out on the runner's 8
/// SMT-paired Sapphire Rapids cores may price it differently. Re-tested with
/// interleaved switch pairs off the Zen family (measurements in the
/// submission note) and flipped on. Transcript-identity is unchanged
/// (`prove_transcript_identical_*`).
#[inline]
...
fn prove_packed_padded_inner<C: Challenger>(
    ...
    // Level 4 ships on too (kill switch FLOCK_NO_ZC_CASCADE5); see
    // `cascade5_off` for the measurement history.
    let use_cascade4 =
