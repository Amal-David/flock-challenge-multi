...
pub fn prove_packed_padded<C: Challenger>(
    ...
    let (proof, claim, _) = prove_packed_padded_inner(
        a_packed, b_packed, c_packed, m, padding, false, None, None, None,
        challenger,
    );
    (proof, claim)
}
...
pub fn prove_packed_padded_capture_s_hat_v_c<C: Challenger>(
    ...
    let (proof, claim, captured) = prove_packed_padded_inner(
        a_packed, b_packed, c_packed, m, padding, true, None, None, None,
        challenger,
    );
    (
        proof,
...
pub fn prove_packed_padded_capture_s_hat_v_c_with_precomputed_ab_and_identity_c<C: Challenger>(
    ...
    prove_packed_padded_capture_s_hat_v_c_with_precomputed_ab_and_identity_c_and_regen(
        a_packed,
        b_packed,
        c_packed,
        c_identity_z,
...
/// [`prove_packed_padded_capture_s_hat_v_c_with_precomputed_ab_and_identity_c`]
/// with an optional round-two a/b octa regenerator (see
/// [`multilinear::AbOctaRegen`]): when supplied, the round-two sweep rebuilds
/// each chunk's packed a/b rows into a task-local tile from the closed form
/// instead of streaming the two packed buffers from DRAM. Value-identical —
/// the regenerated bytes equal the drained bytes by construction.
#[allow(clippy::too_many_arguments)]
pub fn prove_packed_padded_capture_s_hat_v_c_with_precomputed_ab_and_identity_c_and_regen<
    C: Challenger,
>(
    a_packed: &[u8],
    b_packed: &[u8],
    ...
    padding: &PaddingSpec,
    ab_inner: univariate_skip_optimized::Round1AbInner,
    ab_regen: Option<multilinear::AbOctaRegen<'_>>,
    challenger: &mut C,
) -> (ZerocheckProof, ZerocheckClaim, CapturedSHatVC) {
    let (proof, claim, captured) = prove_packed_padded_inner(
        a_packed,
    ...
    for level in 0..n_levels {
        ...
        let (m_even_1, m_even_inf) = if level == 0 && use_nomat {
            ...
            let (m1, mi, la_next) = fold2_from_packed_and_round_pair_lookahead_into_with_eq(
                ...
                &r_next,
                eq_override,
                // Opt-in measurement surface (`FLOCK_N26_REGEN=1`); the
                // cleared ranked environment keeps the incumbent reads.
                if std::env::var_os("FLOCK_N26_REGEN").is_some() {
                    ab_regen
                } else {
                    None
                },
