...
/// Borrow the protocol-fixed inverse-NTT table for `k_skip == K_SKIP`.
///
/// The table depends only on `k_skip`, never on the transcript, so every
/// caller in a worker process wants the same 32 KiB of bytes. Round one has
/// borrowed the cached copy since the static was introduced; this accessor
/// lets witness generation borrow it too instead of rebuilding an identical
/// table on every proof. Byte-for-byte the same table `build_urm_inv_table`
/// returns — the reference is shared read-only, so nothing about the
/// resulting transform changes.
#[inline]
