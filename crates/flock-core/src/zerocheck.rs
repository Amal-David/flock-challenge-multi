...
/// Test-only latches for cascade levels 3 through 5 (rounds 9+10, 11+12, and
/// 13+14). All three levels ship on, so each latch forces the matching level
/// *off*. Either way both routes emit the same transcript — that is what the
...
/// Cascade level 5 (rounds 13+14) uses the same transcript-identical composed
/// double-fold as the preceding cascade levels. At the ranked shape it removes
/// the two remaining fused passes at log sizes 16 and 15. The existing
/// fallback is retained for diagnostics and emergency rollback.
#[inline]
...
fn prove_packed_padded_inner<C: Challenger>(
    ...
    // Levels 3 through 5 (rounds 9+10 through 13+14) extend the same chain.
    // Their parity weights are sampled slots (the seven protocol constants end
    // at k_skip+6), so they are non-zero except with probability 2⁻¹²⁸; the
    // chain simply stops a level earlier otherwise.
...
mod tests {
    ...
    /// double fold) and its cascades (rounds 5+6 through 13+14) each emit
    /// a proof and claim byte-identical to the incumbent route — dense and
    /// BLAKE3-padded (k_log=14, useful=15409), m ∈ {13, 14, 17, 18, 19, 20}
    /// (n_mlv = 7 enables level 2 only, 8 three, 10 four, 12 five, 14 six) — and the
    ...
    fn prove_transcript_identical_with_and_without_lookahead() {
        ...
        for &(m, padded) in &[
            ...
            let arms = [
                ...
                (false, false, false, false, false, true, false), // cascade capped at level 5
                (false, false, false, false, true, true, false),  // cascade capped at level 4
                (false, false, false, true, true, true, false),   // cascade capped at level 3
            ...
            for (k, &(la_off, c2_off, c3_off, c4_off, c5_off, c6_off, nm_off)) in
                arms.iter().enumerate()
            {
