...
#[cfg(all(target_os = "macos", not(test)))]
// Mac-only: glibc's tcache already recycles size-classed allocations; the
// same port measured −3.8% on the x86 ranked runner (public submission
// 8f44e91), so keep the system allocator off-macOS.
