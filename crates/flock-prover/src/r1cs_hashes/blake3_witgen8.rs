...
struct Drain8<'t> {
    ...
    /// Resolved once per octa from the process selector and base alignment.
    wide_nt: [u8; 3],
}
...
/// Resolve one destination's non-temporal store width once per octa. Every
/// production row stride and drain step preserves the base's 64/32-byte
/// alignment, so the hot store helpers need no per-row pointer tests.
#[inline(always)]
fn nt_store_width(dst: *mut u32, enabled: bool) -> u8 {
    if !enabled {
        0
...
pub(crate) unsafe fn build_octa_witness_ab_stream_elide(
    ...
    unsafe {
        ...
        let mut drain = Drain8 {
            ...
            z_nt,
            wide_nt: [
                nt_store_width(z, wide_nt_on),
                nt_store_width(a, wide_nt_on),
                nt_store_width(b, wide_nt_on),
            ],
        };
