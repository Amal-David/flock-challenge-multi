...
use flock_core::zerocheck::univariate_skip_optimized::{
    ...
    round1_ab_inner_window_from_offsets, round1_ab_inner_window_from_offsets_nt2_pair,
    ...
    unsafe fn project_blocks_ranked_hot_offsets(&self, blk: usize, plan: Round1AbWindowPlan, imgs: Round1AbTableImages, rows: RankedRows, off: *const u16) {
        unsafe {
            ...
            while j!=8 {
                ...
                rows.publish_dense(j+1,sa,sb);
                round1_ab_inner_window_from_offsets_nt2_pair(&*off.add(j*ROUND1_AB_OFF_WORDS).cast::<[u16;ROUND1_AB_OFF_WORDS]>(),&*off.add((j+1)*ROUND1_AB_OFF_WORDS).cast::<[u16;ROUND1_AB_OFF_WORDS]>(),self.out.add(blk*64),j,BYTES_PER_BLOCK,plan,imgs);
                j+=2;
            }
        }
    }
