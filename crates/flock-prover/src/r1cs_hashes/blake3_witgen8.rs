...
unsafe fn tr8_chunk(stage: *const V8, w: usize) -> [V8; 8] {
    unsafe {
        #[cfg(all(target_arch = "x86_64", target_feature = "avx512f"))]
        {
            let z01 = _mm512_loadu_si512(stage.add(w).cast::<__m512i>());
            let z23 = _mm512_loadu_si512(stage.add(w + 2).cast::<__m512i>());
            let z45 = _mm512_loadu_si512(stage.add(w + 4).cast::<__m512i>());
            let z67 = _mm512_loadu_si512(stage.add(w + 6).cast::<__m512i>());
            tr8_zmm(z01, z23, z45, z67)
        }
        #[cfg(not(all(target_arch = "x86_64", target_feature = "avx512f")))]
        {
                ...
                if E && (blk <= 1 || blk == 30 || blk == 31) {
                    ...
                    if blk != 31 {
                        #[cfg(all(target_feature = "avx512f", target_feature = "avx512bw"))]
                        {
                            ...
                            for r in 0..8 {
                                store_ranked_stage_line(sa.add(r * STEP_WORDS), a_rows[r]);
                            }
                        }
