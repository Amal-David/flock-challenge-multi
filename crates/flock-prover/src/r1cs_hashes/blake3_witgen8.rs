...
struct StepRows {
    ...
    a_lo: [V8; 8],
    a_hi: [V8; 8],
    /// Pre-computed B rows from the drain's tr8, identical to the bytes that
    /// would have been stored to the staging buffer, so publish emits them
    /// straight from registers instead of reloading `sb`.
    b_lo: [V8; 8],
    b_hi: [V8; 8],
...
impl StepRows {
    ...
    unsafe fn publish(&self, j: usize, _sa: *const u32, _sb: *const u32) {
        unsafe {
            ...
            // B rows are pre-computed by the drain's tr8 (b_lo/b_hi) and
            // cached here, identical to the bytes that used to be stored to
            // the staging buffer — skip the reload from sb.
            emit_pair(
                self.b.add(o),
                self.b_lo[j],
                self.b_hi[j],
    ...
    unsafe fn drain_range_spread(
        ...
        unsafe {
            ...
            for off in (0..words).step_by(STEP_WORDS) {
                ...
                for r in 0..8 {
                    // The staging `sa`/`sb` are only consumed by the
                    // byte-window fallback in `project_blocks` (taken when
                    // `!use_off`). On the offset-eligible path the projection
                    // reads the offset arena instead, so these 32 stores are
                    // dead traffic and are skipped.
                    if !use_off {
