...
/// Physical capacity shared by both rolling schedules. The measured ranked
/// path drains a complete 128-word epoch so the outlined projection crosses
/// the BLAKE3 writer only three times. Cold and partial-provenance paths keep
/// the incumbent 32-word cadence, but place their four successive epochs in
/// disjoint quarters of this ring before wrapping.
const RING_WORDS: usize = 128;
const RANKED_DRAIN_WORDS: usize = RING_WORDS;
const FALLBACK_DRAIN_WORDS: usize = 32;
...
const _RING_GEOMETRY: () = {
    ...
    assert!(RANKED_DRAIN_WORDS == 128);
    assert!(FALLBACK_DRAIN_WORDS == 32);
    assert!(RING_WORDS.is_multiple_of(FALLBACK_DRAIN_WORDS));
    assert!(U32_PER_BLOCK.is_multiple_of(RANKED_DRAIN_WORDS));
    assert!(U32_PER_BLOCK.is_multiple_of(FALLBACK_DRAIN_WORDS));
    // Every epoch boundary is a whole number of drain steps.
    assert!(RANKED_DRAIN_WORDS.is_multiple_of(STEP_WORDS));
    assert!(FALLBACK_DRAIN_WORDS.is_multiple_of(STEP_WORDS));
    assert!(RANKED_DRAIN_WORDS > 16 + PROLOGUE_WORDS);
...
struct W8<'t, const FLUSH: bool, const RANKED: bool> {
    pending: V8,
    stage: *mut V8,
    drain: *mut Drain8<'t>,
}

impl<'t, const FLUSH: bool, const RANKED: bool> W8<'t, FLUSH, RANKED> {
    #[inline(always)]
    ...
    unsafe fn write_word<const WORD: usize>(&mut self, v: V8) {
        unsafe {
            ...
            if FLUSH && RANKED {
                if WORD % RANKED_DRAIN_WORDS == RANKED_DRAIN_WORDS - 1 {
                    // Words 0..15 cannot be published until the final
                    // chaining value is known. The first ranked epoch starts
                    // at word 16; later epochs cover a complete 128 words.
                    if WORD + 1 == RANKED_DRAIN_WORDS {
                        (*self.drain).drain_range::<true>(16, 16, RANKED_DRAIN_WORDS - 16);
                    } else {
                        (*self.drain).drain_range::<true>(
                            WORD + 1 - RANKED_DRAIN_WORDS,
                            0,
                            RANKED_DRAIN_WORDS,
                        );
                    }
                }
            } else if FLUSH && WORD % FALLBACK_DRAIN_WORDS == FALLBACK_DRAIN_WORDS - 1 {
                // Preserve the incumbent 32-word drain cadence for every
                // non-ranked provenance/plan state. The physical ring is
                // larger, so select the completed quarter explicitly.
                if WORD + 1 == FALLBACK_DRAIN_WORDS {
                    (*self.drain).drain_range::<false>(16, 16, FALLBACK_DRAIN_WORDS - 16);
                } else {
                    let base = WORD + 1 - FALLBACK_DRAIN_WORDS;
                    (*self.drain).drain_range::<false>(
                        base,
                        base & (RING_WORDS - 1),
                        FALLBACK_DRAIN_WORDS,
                    );
                }
            }
        }
...
impl Drain8<'_> {
    #[rustfmt::skip]
    #[inline(always)]
    unsafe fn drain_range<const E: bool>(&mut self, base_word: usize, ring_word: usize, words: usize) {
        unsafe {
            self.drain_range_spread::<E>(&self.proj,base_word,ring_word,words);
        }
    }
...
pub(crate) unsafe fn build_octa_witness_ab_stream_elide(
    ...
    unsafe {
        // Only the ranked closed-form source plus all-elide provenance and
        // the AVX-512 offset plan select the long/static schedule. Slice,
        // partial, and cold states retain the incumbent 32-word drain.
        let ranked_static = matches!(&inputs, OctaInputs::Closed { .. })
            && elide == [true; 3]
            && proj.plan.offsets_eligible(2);
        if ranked_static {
            build_octa_witness_ab_stream_elide_impl::<true>(inputs, z, a, b, proj, elide);
        } else {
            build_octa_witness_ab_stream_elide_impl::<false>(inputs, z, a, b, proj, elide);
        }
    }
}

/// Const-specialized body: the measured all-elide route contains only its
/// 128-word drain schedule, while the rollback copy retains the incumbent
/// 32-word cadence. Keeping the selector outside this large straight-line
/// writer also lets LLVM retain compression state across every deleted drain.
#[allow(clippy::too_many_arguments)]
#[inline(never)]
unsafe fn build_octa_witness_ab_stream_elide_impl<const RANKED: bool>(
    inputs: OctaInputs<'_>,
    z: *mut u32,
    a: *mut u32,
    b: *mut u32,
    ...
    unsafe {
        ...
        if !RANKED && FALLBACK_DRAIN_WORDS <= PROLOGUE_WORDS + 16 {
            drain.drain_range::<false>(16, 16, FALLBACK_DRAIN_WORDS - 16);
        }

        let pending_bit = packer_initial_bit(shr_v8::<31>(flags));
        let drain_ptr = &mut drain as *mut Drain8;
        let mut wa = W8::<false, RANKED>::at(ast, pending_bit, drain_ptr);
        ...
        let mut wb = W8::<true, RANKED>::at(bs, packer_initial_bit(one), drain_ptr);
        ...
        if RANKED {
            drain.drain_range::<true>(U32_PER_BLOCK - RANKED_DRAIN_WORDS, 0, RANKED_DRAIN_WORDS);
        } else {
            let base = U32_PER_BLOCK - FALLBACK_DRAIN_WORDS;
            drain.drain_range::<false>(base, base & (RING_WORDS - 1), FALLBACK_DRAIN_WORDS);
        }
        ...
        drain.drain_range::<RANKED>(0, 0, 16);
    }
}
