    ...
    unsafe fn write_word<const WORD: usize>(&mut self, v: V8) {
        unsafe {
            ...
            if FLUSH && WORD % RING_WORDS == RING_WORDS - 1 {
                ...
                // at word 16; later epochs cover their complete RING_WORDS.
...
fn add_carry_parts_v8(x: V8, y: V8) -> (V8, V8, V8) {
    ...
    // The carry (`left & right`) is never read by any caller, so it is not
    // computed here, saving one `vpand` per call (112 per octa witness build).
    let sum = add_v8(x, y);
...
fn wide_nt_enabled() -> bool {
    // Ranked Sapphire Rapids always benefits from 64-byte ZMM streams;
    // keep env gate for local debugging without LazyLock overhead on hot path.
...
///
/// A/B rows are cached directly from the drain's tr8 registers: the staged
/// bytes are both the projection input and the main-buffer payload, so
/// publishing can emit them straight from registers without a reload from
/// L1. Z rows remain spilled in the drain frame.
#[derive(Clone, Copy)]
struct StepRows {
    z: *mut u32,
    a: *mut u32,
    ...
    unsafe fn drain_range_spread(
        ...
        unsafe {
            ...
            for off in (0..words).step_by(STEP_WORDS) {
                ...
                // Transpose the two ring sides first. Rows are cached for
                // publish and for the offset arena; staging stores are dead
                // on the offset-eligible fast path (projection consumes the
                // arena, main buffers are published from the cached rows).
