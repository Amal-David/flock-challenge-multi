...
fn add_carry_parts_v8(x: V8, y: V8) -> (V8, V8, V8) {
    ...
    // the carry-in XOR chain entirely — bit-identical outputs. The carry flag
    // (`left & right`) is unread by every caller, so it is not computed.
    let sum = add_v8(x, y);
    ...
    unsafe fn drain_range_spread(
        ...
        unsafe {
            ...
            for off in (0..words).step_by(STEP_WORDS) {
                ...
                for r in 0..8 {
                    // B rows are read back from `sb` by `StepRows::publish`, so
                    // they must be staged. A rows are published straight from
                    // the `a_lo`/`a_hi` registers and, on the offset-eligible
                    // path, the projection reads the prebuilt offset arena
                    // instead of `sa` — so staging A is dead traffic there.
