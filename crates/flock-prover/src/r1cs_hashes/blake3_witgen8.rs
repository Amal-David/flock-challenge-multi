...
#[inline(always)]
/// `FLOCK_NO_WITGEN_CARRY_ID=1` restores the explicit carry-in intermediate in
/// [`add_carry_parts_v8`]. The carry-in `cin = sum ^ x ^ y` is never returned,
/// and over F2 the two values built from it collapse:
/// `x ^ cin = x ^ sum ^ x ^ y = sum ^ y` and `y ^ cin = sum ^ x`. Computing
/// `left`/`right` directly therefore removes the three-input XOR entirely
/// while producing the identical bits. Resolved once per process.
#[inline(always)]
fn carry_identity_on() -> bool {
    static ON: std::sync::LazyLock<bool> =
        std::sync::LazyLock::new(|| std::env::var_os("FLOCK_NO_WITGEN_CARRY_ID").is_none());
    *ON
}
...
fn add_carry_parts_v8(x: V8, y: V8) -> (V8, V8, V8, V8) {
    let sum = add_v8(x, y);
    if carry_identity_on() {
        // `cin` is a pure intermediate: left = x^cin = sum^y, right = y^cin = sum^x.
        let left = xor_v8(sum, y);
        let right = xor_v8(sum, x);
        let carry = and_v8(left, right);
        return (sum, left, right, carry);
    }
