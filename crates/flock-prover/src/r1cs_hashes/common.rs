...
pub(crate) unsafe fn nt_store_row(src: *const u64, dst: *mut u64) {
    ...
    unsafe {
        use std::arch::x86_64::*;
        // 128 bytes as 8 x 16-byte MOVNTDQ stores. Destinations are 16-byte
        // aligned (F128 backing); staging is only 8-byte aligned, so loads
        // stay unaligned. Callers issue the closing sfence once per batch.
...
/// Order preceding non-temporal stores before the next cross-thread publish.
/// aarch64's `stnp` is ordered by the DMBs in the pool's release atomics; x86
/// WC stores need an explicit `sfence`.
#[inline(always)]
pub(crate) fn nt_publish_fence() {
    #[cfg(target_arch = "x86_64")]
    unsafe {
