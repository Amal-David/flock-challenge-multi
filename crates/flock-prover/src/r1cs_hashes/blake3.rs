...
pub(crate) mod witgen_simd {
    ...
    unsafe fn dump_range<const NT: bool>(stage: *const V4, dst: *mut u32, g0: usize, g1: usize) {
        ...
        if NT {
            crate::r1cs_hashes::common::nt_publish_fence();
        }
    }
...
impl Blake3Setup {
    ...
    fn with_profile_and_rate(
        ...
        // Before any rayon use in this process: pin the global pool to one
        // thread per physical core (see `prover::init_physical_global_pool`).
        crate::prover::init_physical_global_pool();
        assert!(n_blocks >= 1, "n_blocks must be ≥ 1");
