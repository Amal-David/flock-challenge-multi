...
/// Whether witness generation borrows zerocheck's cached `K_SKIP` inverse-NTT
/// table instead of building its own identical copy. Default ON; the OFF arm
/// exists only so one binary can A/B the substitution.
#[inline]
...
fn generate_witness_with_ab_packed_and_round1_inner_impl_tuned(
    ...
    // The inverse-NTT table this witness pass hands to the round-1 window
    // transform is a pure function of `K_SKIP` — no transcript, no seed, no
    // shape. Zerocheck already keeps exactly that table alive for the whole
    // process, so rebuilding a second identical copy here spends ~39 us of
    // single-threaded time inside the witness window on every proof for a
    // buffer the process already owns. Borrow the shared one.
    //
    // The two crates carry independent `K_SKIP` constants that the protocol
    // requires to agree; assert it at compile time before substituting.
    const {
        assert!(K_SKIP == flock_core::zerocheck::K_SKIP);
    }
    // Same-binary rollback: `FLOCK_NO_WITGEN_URM_SHARE=1` restores the local
    // rebuild for A/B screening. Default ON, and the ranked worker's cleared
    // environment can never select the OFF arm.
    let inv_table_owned;
