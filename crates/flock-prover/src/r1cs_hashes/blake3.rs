...
/// enables it for one-process A/B runs). Resolved once per process; the
/// ranked worker's cleared environment leaves it off, so round two keeps the
/// incumbent streamed packed-buffer reads while the mechanism stays in the
/// tree for re-measurement.
