...
/// Opt-in selector for the round-two a/b regeneration (`FLOCK_R2_REGEN=1`
/// enables it for one-process A/B runs). PERMANENTLY opt-in: two independent
/// official readings of the default-on form (both tile-allocation variants)
/// measured ~-46%/-50% despite local A/Bs reading -0.2..-0.6% on identical
/// hardware — the official environment triggers a failure mode local runs do
/// not reproduce. The cleared ranked environment therefore keeps the
/// incumbent streamed packed-buffer reads.
fn r2_regen_enabled() -> bool {
    static ON: std::sync::LazyLock<bool> =
        std::sync::LazyLock::new(|| std::env::var_os("FLOCK_R2_REGEN").is_some());
    *ON
}
