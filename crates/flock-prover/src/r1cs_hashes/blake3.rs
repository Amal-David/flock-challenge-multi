...
fn generate_witness_with_ab_packed_and_round1_inner_impl_tuned(
    ...
    if use_simd && !use_nt && n_total >= 16 {
        generate_round1_inner_hexadeca(
            ...
            &inv_table,
            &padding,
            // First hexadeca wiring: ALL elision OFF. The dump itself is
            // the only untested code this flight runs; every drain elide
            // branch (z zero-tail included) stays dead until the dump is
            // verified green, then lands with the fused-arm follow-up.
            [false, false, false],
            ab_nt,
        );
