...
unsafe fn copy_ranked_seed_row(src: *const F128, dst: *mut F128) {
    use core::arch::x86_64::*;
    for lane in (0..64).step_by(4) {
        let value =
            unsafe { _mm512_loadu_si512(src.add(lane).cast::<__m512i>()) };
        unsafe {
            _mm512_storeu_si512(dst.add(lane).cast::<__m512i>(), value);
        }
    }
}
...
fn generate_witness_and_seed_top_fused(
    ...
    const ODD_ROWS_F128: usize = K_PER_Q * QUADRANTS * 64;
    const ODD_ROW_LINES: usize = ODD_ROWS_F128 * core::mem::size_of::<F128>() / 64;
    ...
    rayon::broadcast(|_| {
        ...
        let mut staging = ctx.take_staging();
        let mut odd_row_lines = uninit_lines(ODD_ROW_LINES);
        let mut z_local_lines = uninit_lines(Z_LOCAL_LINES);
        let mut proj_lines = uninit_lines(STAGE_LINES);
        let odd_rows = odd_row_lines.as_mut_ptr().cast::<F128>();
            ...
            unsafe {
                for octa in 0..OCTAS_PER_Q {
                    ...
                    for kk in 0..2 {
                        ...
                        ctx.seed_group(staging.as_mut_ptr(), k, src, 2, 0);
                        for i in 0..QUADRANTS {
                            copy_ranked_seed_row(
                                z_local.add((8 * kk + 2 * i + 1) * 64),
                                odd_rows.add((k * QUADRANTS + i) * 64),
                            );
                        }
                    }
                }
                ctx.finish_task(staging.as_mut_ptr(), 2 * q);
                for k in 0..K_PER_Q {
                    ctx.seed_group(
                        staging.as_mut_ptr(),
                        k,
                        odd_rows.add(k * QUADRANTS * 64),
                        1,
                        0,
                    );
                }
                ctx.finish_task(staging.as_mut_ptr(), 2 * q + 1);
