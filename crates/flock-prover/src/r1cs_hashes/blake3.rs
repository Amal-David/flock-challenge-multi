...
thread_local! {
    ...
    static R2_REGEN_TLS: std::cell::RefCell<Option<(Vec<u8>, Vec<u8>)>> =
...
/// Resolved once per process; the ranked worker's cleared environment always
/// takes the regeneration path.
...
fn make_r2_regen(
    ...
    Some(Box::new(move |first_block: usize, a_dst: *mut u8, b_dst: *mut u8| {
        R2_REGEN_TLS.with(|tls| {
            ...
            let (stage_v, zscrap) = tls.get_or_insert_with(|| {
                (
