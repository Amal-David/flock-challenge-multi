...
//! returned to the system allocator. The ranked worker performs an untimed
//! warm proof with the same allocation pattern, so the timed proof reuses
//! resident pages for large allocations not already handled by the typed
//! scratch pools.
//!
//! Ported from the mac track's `flock-challenge` prover (arch-independent:
//! pure `std::alloc`, no macOS-specific APIs) to close the same warm-up ->
//! timed-proof allocation gap on the x86_64 Linux ranked runner.
...
// Both macOS libmalloc and glibc/musl malloc on Linux provide at least
// 16-byte alignment at these sizes, and layouts requiring larger alignment
// bypass the recycler.
