...
fn prove_fast_ligerito_from_witness_inner<Ch: Challenger>(
    ...
    let pcs_open = std::thread::scope(|scope| {
        let stash = crate::proof_io::pre_encode_enabled().then(|| {
            scope.spawn(|| {
                crate::proof_io::stash_pre_encoded_prefix(
                    &commitment,
                    &zc_proof,
                    &lc_proof,
                );
        ...
        if let Some(handle) = stash {
            // Finished long ago (µs vs the ~20 ms open); joining before the
            // scope exits preserves the publish-before-consume ordering.
            let _ = handle.join();
        }
        pcs_open
    });
